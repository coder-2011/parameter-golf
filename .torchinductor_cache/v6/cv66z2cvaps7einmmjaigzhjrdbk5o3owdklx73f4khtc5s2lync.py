...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/tn/ctnudunut4kwgx4k7jhogb32orpwnfjk53uhbrttyomuaeoisdfn.py
# Topologically Sorted Source Nodes: [mul, linear], Original ATen: [aten.mul, aten.t]
# Source node to ATen node mapping:
#   linear => permute
#   mul => mul
# Graph fragment:
#   %primals_1 : Tensor "bf16[8192, 384][384, 1]cuda:7" = PlaceHolder[target=primals_1]
#   %mul : Tensor "bf16[8192, 384][384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_1, 1.0), kwargs = {})
#   %permute : Tensor "bf16[384, 8192][1, 384]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%mul, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/3r/c3r4cgawvucwrvvk2qkezz4mrggn5qhvpbzzdqxwif6qwqwjvp2q.py
...
#   %sum_1 : Tensor "f32[131072, 1][1, 131072]cuda:7" = PlaceHolder[target=sum_1]
...
#   %tanh : Tensor "f32[131072, 8192][8192, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.tanh.default](args = (%div,), kwargs = {})
#   %mul_tensor : Tensor "f32[131072, 8192][8192, 1]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tanh, 1), kwargs = {})
#   %amax_default : Tensor "f32[131072, 1][1, 1]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.amax.default](args = (%mul_tensor, [1], True), kwargs = {})
...
#   %mul_tensor_1 : Tensor "f32[131072, 8192][8192, 1]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sub_tensor, 30.0), kwargs = {})
#   %exp : Tensor "f32[131072, 8192][8192, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.exp.default](args = (%mul_tensor_1,), kwargs = {})
#   %sum_1 : Tensor "f32[131072, 1][1, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%exp, [1], True), kwargs = {})
#   %log : Tensor "f32[131072, 1][1, 1]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.log.default](args = (%sum_1,), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ik/cikx4qdbiwq5i4dezewba6fo5yn6ikgtawjro74bm4hthsmsoh6d.py
...
#   %ne : Tensor "b8[131072][1]cuda:7"[num_users=3] = call_function[target=torch.ops.aten.ne.Scalar](args = (%primals_3, -100), kwargs = {})
...
#   %where : Tensor "i64[131072][1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%ne, %primals_3, %full_default), kwargs = {})
#   %unsqueeze : Tensor "i64[131072, 1][1, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%where, 1), kwargs = {})
#   %gather : Tensor "f32[131072, 1][1, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.gather.default](args = (%sub_1, 1, %unsqueeze), kwargs = {})
#   %squeeze : Tensor "f32[131072][1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.squeeze.dim](args = (%gather, 1), kwargs = {})
#   %neg : Tensor "f32[131072][1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.neg.default](args = (%squeeze,), kwargs = {})
...
#   %where_1 : Tensor "f32[131072][1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%ne, %neg, %full_default_1), kwargs = {})
#   %sum_2 : Tensor "i64[][]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.sum.default](args = (%ne,), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/cz/cczjalkmjyo33g2ofpubp5c7nyzn75t72ngfyxpglk6gbpfrqruw.py
# Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.sub, aten._log_softmax, aten.nll_loss_forward]
# Source node to ATen node mapping:
...
#   truediv => div
# Graph fragment:
#   %buf10 : Tensor "f32[16][1]cuda:7" = PlaceHolder[target=buf10]
#   %buf6 : Tensor "i64[16][1]cuda:7" = PlaceHolder[target=buf6]
...
#   %where_1 : Tensor "f32[131072][1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%ne, %neg, %full_default_1), kwargs = {})
#   %sum_2 : Tensor "i64[][]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.sum.default](args = (%ne,), kwargs = {})
#   %convert_element_type_3 : Tensor "f32[][]cuda:7"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%sum_2, torch.float32), kwargs = {})
#   %sum_3 : Tensor "f32[][]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.sum.default](args = (%where_1,), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(7):
            ...
            triton_poi_fused_mul_t_0.run(primals_1, buf0, 3145728, stream=stream7)
            del primals_1
            ...
            # Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.amax, aten.sub, aten._log_softmax]
            stream7 = get_raw_stream(7)
            triton_red_fused__log_softmax__to_copy_amax_div_mul_sub_tanh_1.run(buf4, buf1, buf2, 131072, 8192, stream=stream7)
            buf6 = empty_strided_cuda((16, ), (1, ), torch.int64)
            buf10 = empty_strided_cuda((16, ), (1, ), torch.float32)
            # Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.sub, aten._log_softmax, aten.nll_loss_forward]
            stream7 = get_raw_stream(7)
            triton_red_fused__log_softmax__to_copy_div_mul_nll_loss_forward_sub_tanh_2.run(primals_3, buf1, buf2, buf4, buf6, buf10, 16, 8192, stream=stream7)
            buf11 = empty_strided_cuda((), (), torch.float32)
            buf8 = empty_strided_cuda((), (), torch.float32)
            buf12 = buf11; del buf11  # reuse
            # Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.sub, aten._log_softmax, aten.nll_loss_forward]
            stream7 = get_raw_stream(7)
            triton_per_fused__log_softmax__to_copy_div_mul_nll_loss_forward_sub_tanh_3.run(buf12, buf10, buf6, buf8, 1, 16, stream=stream7)
            del buf10
            del buf6
...
def get_args():
    ...
    primals_1 = rand_strided((8192, 384), (384, 1), device='cuda:7', dtype=torch.bfloat16)
