...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/7y/c7ytz2ahn3hd7mu6oej4b3mpz4afybyydhugzrx4ak64xr53vimb.py
...
# Graph fragment:
#   %full_default_2 : Tensor "f32[8192, 384][384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([8192, 384], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/iy/ciyaojygi4vmnzm42fri7pl2tubuyu7vg373lmdrqz2qiejm7kwy.py
...
#   %index_put : Tensor "f32[8192, 384][384, 1]cuda:4" = PlaceHolder[target=index_put]
#   %convert_element_type : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%tangents_1, torch.float32), kwargs = {})
#   %eq : Tensor "b8[128, 1024][1024, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%primals_5, -1), kwargs = {})
#   %unsqueeze_2 : Tensor "b8[128, 1024, 1][1024, 1, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
...
#   %where : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_2, %full_default_1, %convert_element_type), kwargs = {})
#   %full_default_2 : Tensor "f32[8192, 384][384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([8192, 384], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   %index_put : Tensor "f32[8192, 384][384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_2, [%primals_5], %where, True), kwargs = {})
#   return %buf1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/xi/cxiptvzdiy6skfgn2wohb276reig2n3bydtxqmthmdhtkf2ghh6q.py
...
# Graph fragment:
#   %buf1 : Tensor "f32[8192, 384][384, 1]cuda:4" = PlaceHolder[target=buf1]
#   %convert_element_type_1 : Tensor "bf16[8192, 384][384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            triton_poi_fused_embedding_dense_backward_0.run(buf0, 3145728, stream=stream4)
            # Topologically Sorted Source Nodes: [convert_element_type, eq, unsqueeze_2, full_default_1, where, full_default_2, index_put], Original ATen: [aten.embedding_dense_backward]
            stream4 = get_raw_stream(4)
            triton_poi_fused_embedding_dense_backward_1.run(primals_5, tangents_1, buf0, 131072, 384, stream=stream4)
            del primals_5
            del tangents_1
            buf2 = empty_strided_cuda((8192, 384), (384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [convert_element_type_1], Original ATen: [aten.embedding_dense_backward]
            stream4 = get_raw_stream(4)
            triton_poi_fused_embedding_dense_backward_2.run(buf0, buf2, 3145728, stream=stream4)
            del buf0
