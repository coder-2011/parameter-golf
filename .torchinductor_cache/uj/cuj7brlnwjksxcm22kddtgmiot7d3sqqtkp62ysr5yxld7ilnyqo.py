...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/pv/cpvh32uxqtpxscy7hsnrihcysz4biwcjfa5z7vfbanyyq7wqdext.py
...
# Graph fragment:
#   %full_default_4 : Tensor "f32[196608, 128][128, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([196608, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:5, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/3u/c3ugfd7zqxfgvzvcwn2yqlhksnizrmo2lxo3w5zdfjmzkphe4bak.py
...
#   %index_put : Tensor "f32[196608, 128][128, 1]cuda:5" = PlaceHolder[target=index_put]
...
#   %full_default_4 : Tensor "f32[196608, 128][128, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([196608, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:5, pin_memory: False})
#   %index_put : Tensor "f32[196608, 128][128, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_4, [%primals_1], %where, True), kwargs = {})
#   return %buf44
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/b5/cb5v5zq4tulfsbktrpmzqkv3mnl5bqibftxu4b7ng7hkeqeieudz.py
...
# Graph fragment:
#   %buf44 : Tensor "f32[196608, 128][128, 1]cuda:5" = PlaceHolder[target=buf44]
#   %convert_element_type_80 : Tensor "bf16[196608, 128][128, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(5):
            ...
            # Topologically Sorted Source Nodes: [full_default_4], Original ATen: [aten.embedding_dense_backward]
            stream5 = get_raw_stream(5)
            triton_poi_fused_embedding_dense_backward_12.run(buf43, 25165824, stream=stream5)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_42, unsqueeze_4, expand, convert_element_type_default, eq, unsqueeze_5, full_default_3, where, full_default_4, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream5 = get_raw_stream(5)
            ...
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream5 = get_raw_stream(5)
            triton_poi_fused_embedding_dense_backward_14.run(buf43, buf45, 25165824, stream=stream5)
            del buf43
