...
_tensor_constant0 = None  # device(type='cuda', index=4) torch.int64 (12,) (1,) 7bb0a21c7750
_tensor_constant1 = None  # device(type='cuda', index=4) torch.int64 (12,) (1,) 7bb0a21d8910
...
#   %embedding : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_6, %primals_5), kwargs = {})
#   return %embedding
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/7i/c7izfytjd2czljwis6gbospy6n3srq52fjj4hxgfsrvlzobmxayc.py
...
#   %select : Tensor "i64[128, 12][12288, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%empty, 1, 0), kwargs = {})
#   %copy : Tensor "i64[128, 12][12288, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%select, %full_default), kwargs = {})
#   %select_scatter_default : Tensor "i64[128, 1024, 12][12288, 12, 1]cuda:4"[num_users=2] = call_function[target=torch.ops.aten.select_scatter.default](args = (%empty, %copy, 1, 0), kwargs = {})
...
#   %mul : Tensor "i64[128, 1023, 12][12276, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %lift_fresh_copy), kwargs = {})
...
#   %mul_1 : Tensor "i64[128, 1023, 12][12276, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze_1, %lift_fresh_copy_1), kwargs = {})
#   %bitwise_xor : Tensor "i64[128, 1023, 12][12276, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.bitwise_xor.Tensor](args = (%mul, %mul_1), kwargs = {})
#   %remainder : Tensor "i64[128, 1023, 12][12276, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
#   %add : Tensor "i64[128, 1023, 12][12276, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%remainder, 1), kwargs = {})
#   %slice_4 : Tensor "i64[128, 1023, 12][12288, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%select_scatter_default, 1, 1, 9223372036854775807), kwargs = {})
#   %copy_1 : Tensor "i64[128, 1023, 12][12288, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%slice_4, %add), kwargs = {})
#   %slice_scatter_default : Tensor "i64[128, 1024, 12][12288, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.slice_scatter.default](args = (%select_scatter_default, %copy_1, 1, 1, 9223372036854775807), kwargs = {})
...
#   %mul_2 : Tensor "i64[12][1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%iota, 16384), kwargs = {})
#   %add_1 : Tensor "i64[128, 1024, 12][12288, 12, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            # Topologically Sorted Source Nodes: [getitem_4, getitem_5, tensor, tensor_1, setitem, getitem_6, mul, getitem_7, mul_1, bitwise_xor, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.slice, aten.lift_fresh, aten.select, aten.fill, aten.unsqueeze, aten.mul, aten.bitwise_xor, aten.remainder, aten.add, aten.copy, aten.arange]
            stream4 = get_raw_stream(4)
            triton_poi_fused_add_arange_bitwise_xor_copy_fill_lift_fresh_mul_remainder_select_slice_unsqueeze_1.run(primals_5, _tensor_constant0, _tensor_constant1, buf2, 131072, 12, stream=stream4)
