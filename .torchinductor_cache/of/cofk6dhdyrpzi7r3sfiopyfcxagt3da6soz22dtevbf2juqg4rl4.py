...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/eu/ceu2fdb5pb3sb2gkc3r2k4n6gh7edxmkcuakhch5x6gxfwc7edvu.py
...
#   %primals_2 : Tensor "bf16[131072, 96][96, 1]cuda:2" = PlaceHolder[target=primals_2]
#   %sum_1 : Tensor "f32[128, 1024, 96][98304, 96, 1]cuda:2" = PlaceHolder[target=sum_1]
#   %embedding : Tensor "bf16[128, 1024, 8, 96][786432, 768, 96, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_2, %primals_1), kwargs = {})
#   %sum_1 : Tensor "f32[128, 1024, 96][98304, 96, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%embedding, [-2]), kwargs = {dtype: torch.float32})
#   %mul : Tensor "f32[128, 1024, 96][98304, 96, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 96][98304, 96, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/5n/c5n3ps77umbojbe7zundaqxra6dodvaeoevoyai2vhsnwsgwszpn.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
# Source node to ATen node mapping:
#   linear => convert_element_type_default_7, permute
# Graph fragment:
#   %primals_3 : Tensor "bf16[384, 96][96, 1]cuda:2" = PlaceHolder[target=primals_3]
#   %convert_element_type_default_7 : Tensor "bf16[384, 96][96, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_3, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[96, 384][1, 96]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_7, [1, 0]), kwargs = {})
#   return %permute
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_per_fused__to_copy_embedding_mul_sum_0.run(primals_1, primals_2, buf2, 12582912, 8, stream=stream2)
            del primals_2
            buf1 = empty_strided_cuda((96, 384), (1, 96), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_1.run(primals_3, buf1, 36864, stream=stream2)
            del primals_3
...
def get_args():
    ...
    primals_2 = rand_strided((131072, 96), (96, 1), device='cuda:2', dtype=torch.bfloat16)
    primals_3 = rand_strided((384, 96), (96, 1), device='cuda:2', dtype=torch.bfloat16)
