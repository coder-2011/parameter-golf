...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/4m/c4mh32l4yyvj3vue3lgt2jlj36pxhaetzxbf2kjeo3erfi5eqhtb.py
...
# Graph fragment:
#   %primals_1 : Tensor "bf16[384, 1536][1536, 1]cuda:2" = PlaceHolder[target=primals_1]
#   %convert_element_type_default_1 : Tensor "bf16[384, 1536][1536, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_1, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[1536, 384][1, 1536]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_1, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/vq/cvq5romwu5bxwjt6xnbdb7ifrisxmbtzndssitwzsdqh2z26cyrt.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
# Source node to ATen node mapping:
#   linear => convert_element_type_2
# Graph fragment:
#   %primals_2 : Tensor "f32[128, 1024, 1536][1572864, 1536, 1]cuda:2" = PlaceHolder[target=primals_2]
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 1536][1572864, 1536, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_2, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/h4/ch44sv44kf6ujmqfrlgyz4yfdgdfxtdkf6iqs4jyrt6bras5m2id.py
...
#   mul_1 => mul_1
# Graph fragment:
#   %primals_5 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2" = PlaceHolder[target=primals_5]
...
#   %mul : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_3, %primals_4), kwargs = {})
#   %add : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_5, %mul), kwargs = {})
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_6, %view_1), kwargs = {})
#   %add_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%add, %mul_1), kwargs = {})
#   return %add_1
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused__to_copy_t_0.run(primals_1, buf0, 589824, stream=stream2)
            del primals_1
            buf1 = empty_strided_cuda((128, 1024, 1536), (1572864, 1536, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_1.run(primals_2, buf1, 201326592, stream=stream2)
            del primals_2
            ...
            # Topologically Sorted Source Nodes: [linear, mul, add, mul_1, add_1], Original ATen: [aten._unsafe_view, aten.mul, aten.add]
            stream2 = get_raw_stream(2)
            triton_poi_fused__unsafe_view_add_mul_2.run(primals_5, primals_3, primals_4, primals_6, buf2, buf3, 50331648, stream=stream2)
            del primals_5
...
def get_args():
    ...
    primals_2 = rand_strided((128, 1024, 1536), (1572864, 1536, 1), device='cuda:2', dtype=torch.float32)
    ...
    primals_5 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:2', dtype=torch.float32)
