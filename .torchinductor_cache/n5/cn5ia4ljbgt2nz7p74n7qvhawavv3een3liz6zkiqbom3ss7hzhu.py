...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/pi/cpiticsx47k37glufvsgv6363nqca4vlywbwzbnb3m7quvz34sp5.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/5s/c5sqq3j5xsfvb7rvympeemja4txqixomykvtlouxpawsvikiqkob.py
...
#   %remainder : Tensor "i64[128, 1023, 8][8184, 8, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
...
#   %add_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/fr/cfr25iqz26clf2f34wxhq5s6c7ygghrg73yozw6mq52lps42qx37.py
...
#   %embedding : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_6, %primals_5), kwargs = {})
#   return %embedding
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_bitwise_xor_lift_fresh_mul_slice_unsqueeze_0.run(primals_5, buf2, 130944, 8, stream=stream2)
            buf3 = empty_strided_cuda((128, 1024, 8), (8192, 8, 1), torch.int64)
            # Topologically Sorted Source Nodes: [setitem, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.lift_fresh, aten.select, aten.fill, aten.remainder, aten.add, aten.slice, aten.copy, aten.arange, aten.mul]
            stream2 = get_raw_stream(2)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_1.run(buf2, buf3, 1048576, stream=stream2)
            del buf2
            buf0 = empty_strided_cuda((128, 1024, 384), (393216, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream2 = get_raw_stream(2)
            triton_poi_fused_embedding_2.run(primals_5, primals_6, buf0, 131072, 384, stream=stream2)
            del primals_6
