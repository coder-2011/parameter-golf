...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/yx/cyxwvohl4dzkiqrhkeerm7hxc7tcfyvr5caswmfj6uejpsngqh62.py
...
# Graph fragment:
#   %full_default_5 : Tensor "f32[32768, 128][128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([32768, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/xz/cxzky346lgrwxby3p4hajgcoxbltl4jwcb3xzz4bbo336stcci22.py
...
#   where => where
# Graph fragment:
#   %add_1 : Tensor "i64[128, 1024, 4][4096, 4, 1]cuda:4" = PlaceHolder[target=add_1]
#   %mm_13 : Tensor "bf16[131072, 128][128, 1]cuda:4" = PlaceHolder[target=mm_13]
#   %index_put : Tensor "f32[32768, 128][128, 1]cuda:4" = PlaceHolder[target=index_put]
...
#   %mul_45 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.5), kwargs = {})
...
#   %expand : Tensor "f32[128, 1024, 4, 128][131072, 128, 0, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_6, [128, 1024, 4, 128]), kwargs = {})
#   %convert_element_type_default : Tensor "f32[128, 1024, 4, 128][524288, 512, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%expand, torch.float32), kwargs = {})
#   %eq : Tensor "b8[128, 1024, 4][4096, 4, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%add_1, -1), kwargs = {})
#   %unsqueeze_7 : Tensor "b8[128, 1024, 4, 1][4096, 4, 1, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
#   %full_default_4 : Tensor "f32[][]cuda:4"[num_users=2] = call_function[target=torch.ops.aten.full.default](args = ([], 0.0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   %where : Tensor "f32[128, 1024, 4, 128][524288, 512, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_7, %full_default_4, %convert_element_type_default), kwargs = {})
#   %full_default_5 : Tensor "f32[32768, 128][128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([32768, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   %index_put : Tensor "f32[32768, 128][128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_5, [%add_1], %where, True), kwargs = {})
#   return %buf43
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ht/chtkav3fcdjsemf2rpxc2mycxfff5uh6ifa7hwjafcig46haueey.py
...
# Graph fragment:
#   %buf43 : Tensor "f32[32768, 128][128, 1]cuda:4" = PlaceHolder[target=buf43]
#   %convert_element_type_80 : Tensor "bf16[32768, 128][128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            # Topologically Sorted Source Nodes: [full_default_5], Original ATen: [aten.embedding_dense_backward]
            stream4 = get_raw_stream(4)
            triton_poi_fused_embedding_dense_backward_13.run(buf42, 4194304, stream=stream4)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_45, unsqueeze_6, expand, convert_element_type_default, eq, unsqueeze_7, full_default_4, where, full_default_5, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream4 = get_raw_stream(4)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_14.run(add_1, buf41, buf42, 524288, 128, stream=stream4)
            del add_1
            del buf41
            buf44 = empty_strided_cuda((32768, 128), (128, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream4 = get_raw_stream(4)
            triton_poi_fused_embedding_dense_backward_15.run(buf42, buf44, 4194304, stream=stream4)
            del buf42
...
def get_args():
    ...
    add_1 = rand_strided((128, 1024, 4), (4096, 4, 1), device='cuda:4', dtype=torch.int64)
