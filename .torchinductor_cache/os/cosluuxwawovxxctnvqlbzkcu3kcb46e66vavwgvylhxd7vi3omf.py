...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/az/cazf7wyr3p6z637d6l44yjxwuqam5edscw65wbrduvgxxzb3vx2z.py
...
#   %primals_7 : Tensor "bf16[65536, 128][128, 1]cuda:4" = PlaceHolder[target=primals_7]
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:4" = PlaceHolder[target=sum_1]
#   %embedding_1 : Tensor "bf16[128, 1024, 8, 128][1048576, 1024, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_7, %add_1), kwargs = {})
...
#   %mul_3 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            # Topologically Sorted Source Nodes: [embedding_1, sum_1, mul_3, linear], Original ATen: [aten.embedding, aten.sum, aten.mul, aten._to_copy]
            stream4 = get_raw_stream(4)
            triton_per_fused__to_copy_embedding_mul_sum_3.run(buf3, primals_7, buf6, 16777216, 8, stream=stream4)
            del primals_7
...
def get_args():
    ...
    primals_7 = rand_strided((65536, 128), (128, 1), device='cuda:4', dtype=torch.bfloat16)
