...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6y/c6ycofyx7waoa6koxvyktk2zmsztp3w2tg63er6jr5zvtcnfsxdv.py
...
#   %primals_2 : Tensor "bf16[131072, 128][128, 1]cuda:4" = PlaceHolder[target=primals_2]
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:4" = PlaceHolder[target=sum_1]
#   %embedding : Tensor "bf16[128, 1024, 8, 128][1048576, 1024, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_2, %primals_1), kwargs = {})
...
#   %mul : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            triton_per_fused__to_copy_embedding_mul_sum_0.run(primals_1, primals_2, buf2, 16777216, 8, stream=stream4)
            del primals_2
...
def get_args():
    ...
    primals_2 = rand_strided((131072, 128), (128, 1), device='cuda:4', dtype=torch.bfloat16)
