...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/vj/cvjmr6edp2mvtfxpa6vedhh56hp4rayxc2bgd3twt75uv5qgyh3e.py
...
# Graph fragment:
#   %full_default_2 : Tensor "f32[8192, 384][384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([8192, 384], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:1, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/y6/cy6wsthpnnz6ncyphh4wgov353fam552pnrdcratj2lgocfn2w3m.py
...
#   %index_put : Tensor "f32[8192, 384][384, 1]cuda:1" = PlaceHolder[target=index_put]
#   %convert_element_type : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%tangents_1, torch.float32), kwargs = {})
#   %eq : Tensor "b8[128, 1024][1024, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%primals_5, -1), kwargs = {})
#   %unsqueeze_2 : Tensor "b8[128, 1024, 1][1024, 1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
...
#   %where : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_2, %full_default_1, %convert_element_type), kwargs = {})
#   %full_default_2 : Tensor "f32[8192, 384][384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([8192, 384], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:1, pin_memory: False})
#   %index_put : Tensor "f32[8192, 384][384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_2, [%primals_5], %where, True), kwargs = {})
#   return %buf1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/bj/cbjsmzqacgxbjuokwwgm7ajwa65mv54oddqueqiniqt3zekmhphp.py
...
# Graph fragment:
#   %buf1 : Tensor "f32[8192, 384][384, 1]cuda:1" = PlaceHolder[target=buf1]
#   %convert_element_type_1 : Tensor "bf16[8192, 384][384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            triton_poi_fused_embedding_dense_backward_0.run(buf0, 3145728, stream=stream1)
            # Topologically Sorted Source Nodes: [convert_element_type, eq, unsqueeze_2, full_default_1, where, full_default_2, index_put], Original ATen: [aten.embedding_dense_backward]
            stream1 = get_raw_stream(1)
            triton_poi_fused_embedding_dense_backward_1.run(primals_5, tangents_1, buf0, 131072, 384, stream=stream1)
            del primals_5
            del tangents_1
            buf2 = empty_strided_cuda((8192, 384), (384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [convert_element_type_1], Original ATen: [aten.embedding_dense_backward]
            stream1 = get_raw_stream(1)
            triton_poi_fused_embedding_dense_backward_2.run(buf0, buf2, 3145728, stream=stream1)
            del buf0
