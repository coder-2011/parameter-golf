...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/m6/cm6ehwz64ws52xkk6ar2ig3mq2fpg6zcjdbsih5zefteyzxeeqbm.py
...
#   %mul_3 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tangents_1, %view_1), kwargs = {})
#   %sum_1 : Tensor "f32[1, 1, 384][384, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%mul_3, [0, 1], True), kwargs = {dtype: torch.float32})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/oj/coj7sktdj6gdsw5jisiotyuitvkbsivnsj2xlzbgquejreb6xb4c.py
...
#   %mul_2 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tangents_1, %primals_6), kwargs = {})
#   %convert_element_type_5 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_2, torch.bfloat16), kwargs = {})
#   %mul_4 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tangents_1, %primals_3), kwargs = {})
#   %convert_element_type_6 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_4, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/bn/cbnrvcoii2mdpwkswwsmxkrdhf6awiu4fwrkc55nlzakptnivrsj.py
...
#   view_3 => view_3
# Graph fragment:
#   %mm_2 : Tensor "bf16[131072, 1536][1536, 1]cuda:0" = PlaceHolder[target=mm_2]
#   %view_3 : Tensor "bf16[128, 1024, 1536][1572864, 1536, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm_2, [128, 1024, 1536]), kwargs = {})
#   %convert_element_type_11 : Tensor "f32[128, 1024, 1536][1572864, 1536, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_3, torch.float32), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(0):
            ...
            triton_red_fused__unsafe_view_mul_sum_0.run(tangents_1, mm, primals_4, buf0, buf2, 134016, 376, stream=stream0)
            del mm
            del primals_4
            ...
            # Topologically Sorted Source Nodes: [mul_2, convert_element_type_5, mul_4, convert_element_type_6], Original ATen: [aten.mul, aten._to_copy]
            stream0 = get_raw_stream(0)
            triton_poi_fused__to_copy_mul_2.run(tangents_1, primals_3, primals_6, buf4, buf5, 50331648, stream=stream0)
            del primals_3
            del primals_6
            ...
            # Topologically Sorted Source Nodes: [view_3, convert_element_type_11], Original ATen: [aten.view, aten._to_copy]
            stream0 = get_raw_stream(0)
            triton_poi_fused__to_copy_view_3.run(buf7, buf8, 201326592, stream=stream0)
            del buf7
...
def get_args():
    ...
    view = rand_strided((131072, 1536), (1536, 1), device='cuda:0', dtype=torch.bfloat16)
    mm = rand_strided((131072, 384), (384, 1), device='cuda:0', dtype=torch.bfloat16)
    permute_3 = rand_strided((384, 1536), (1536, 1), device='cuda:0', dtype=torch.bfloat16)
