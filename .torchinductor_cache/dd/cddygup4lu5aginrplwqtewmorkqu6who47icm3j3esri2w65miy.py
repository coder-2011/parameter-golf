...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/sp/cspysaebbam47sskdvp5udh5kwpov2i3hcindljqqeowellpolku.py
# Topologically Sorted Source Nodes: [zeros], Original ATen: [aten.zeros]
# Source node to ATen node mapping:
#   zeros => full_default
# Graph fragment:
#   %full_default : Tensor "bf16[350, 1000, 384][384000, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([350, 1000, 384], 0), kwargs = {dtype: torch.bfloat16, layout: torch.strided, device: cuda:7, pin_memory: False})
#   return %full_default
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(7):
            ...
            triton_poi_fused_zeros_0.run(buf0, 134400000, stream=stream7)
        return (buf0, )
