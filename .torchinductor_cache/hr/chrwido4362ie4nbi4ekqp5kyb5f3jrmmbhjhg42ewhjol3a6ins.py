...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ec/cecrit3imranuuta363n5skjojnahv7365rhdojrq7fkd3a5ok5w.py
...
#   %primals_2 : Tensor "bf16[393216, 128][128, 1]cuda:0" = PlaceHolder[target=primals_2]
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:0" = PlaceHolder[target=sum_1]
#   %embedding : Tensor "bf16[128, 1024, 12, 128][1572864, 1536, 128, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_2, %primals_1), kwargs = {})
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%embedding, [-2]), kwargs = {dtype: torch.float32})
#   %mul : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.28867513459481287), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 128][131072, 128, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/pg/cpg33hsqldjwsvhxinsw4svezxypyeo7sisoehcnpkyiitpjnmao.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
# Source node to ATen node mapping:
#   linear => convert_element_type_default_7, permute
# Graph fragment:
#   %primals_3 : Tensor "bf16[384, 128][128, 1]cuda:0" = PlaceHolder[target=primals_3]
#   %convert_element_type_default_7 : Tensor "bf16[384, 128][128, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_3, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(0):
            ...
            triton_per_fused__to_copy_embedding_mul_sum_0.run(primals_1, primals_2, buf2, 16777216, 12, stream=stream0)
            del primals_2
            buf1 = empty_strided_cuda((128, 384), (1, 128), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
            stream0 = get_raw_stream(0)
            triton_poi_fused__to_copy_t_1.run(primals_3, buf1, 49152, stream=stream0)
            del primals_3
...
def get_args():
    ...
    primals_2 = rand_strided((393216, 128), (128, 1), device='cuda:0', dtype=torch.bfloat16)
    primals_3 = rand_strided((384, 128), (128, 1), device='cuda:0', dtype=torch.bfloat16)
