...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/n4/cn4z5prsaraz25i6ucojybzp7n5ugyswg3ky2z5abxcwl3t7bey3.py
...
#   %remainder : Tensor "i64[512, 999, 8][7992, 8, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
...
#   %mul_2 : Tensor "i64[8][1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%iota, 16384), kwargs = {})
...
#   %embedding : Tensor "bf16[512, 1000, 384][384000, 384, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%arg5_1, %arg4_1), kwargs = {})
#   return %embedding
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            # Topologically Sorted Source Nodes: [setitem, setitem_1, remainder, add, arange, mul_2, add_1], Original ATen: [aten.select, aten.lift_fresh, aten.fill, aten.slice, aten.remainder, aten.add, aten.copy, aten.arange, aten.mul]
            stream4 = get_raw_stream(4)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_1.run(buf1, buf2, 4096000, stream=stream4)
            del buf1
            buf3 = empty_strided_cuda((512, 1000, 384), (384000, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream4 = get_raw_stream(4)
            triton_poi_fused_embedding_2.run(arg4_1, arg5_1, buf3, 512000, 384, stream=stream4)
            del arg4_1
            del arg5_1
