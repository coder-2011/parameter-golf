...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6z/c6zw4lyewulgm3ehdzl2imnvnufkttfxtpre3ctg56p3jb2bmzof.py
# Topologically Sorted Source Nodes: [zeros], Original ATen: [aten.zeros]
# Source node to ATen node mapping:
#   zeros => full_default
# Graph fragment:
#   %full_default : Tensor "bf16[512, 1000, 384][384000, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([512, 1000, 384], 0), kwargs = {dtype: torch.bfloat16, layout: torch.strided, device: cuda:2, pin_memory: False})
#   return %full_default
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_zeros_0.run(buf0, 196608000, stream=stream2)
        return (buf0, )
