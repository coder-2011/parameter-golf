...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6j/c6j4hwhdfdd47ntzdcjzp2sjz6zffcmbhcn3hww5w7jwknh5le54.py
...
# Graph fragment:
#   %primals_1 : Tensor "bf16[1024, 384][384, 1]cuda:1" = PlaceHolder[target=primals_1]
#   %convert_element_type_default_5 : Tensor "bf16[1024, 384][384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_1, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[384, 1024][1, 384]cuda:1"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_5, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/tb/ctbwncsitvfltk4nfzrdyzqeszoaebdm72d3l7xsbhzlmgxo5fvr.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/h7/ch75dyyhf5lgjs4s2hct4ubktodizp4gs7frdlfeqsza37a2r334.py
...
#   %gt : Tensor "b8[128, 1024, 1024][1048576, 1024, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.gt.Scalar](args = (%convert_element_type_5, 0), kwargs = {})
...
#   %pow_1 : Tensor "f32[128, 1024, 1024][1048576, 1024, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%where, 2), kwargs = {})
#   %convert_element_type_10 : Tensor "bf16[128, 1024, 1024][1048576, 1024, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%pow_1, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/7x/c7xwyvratavct6xpv3ycqlpomafynq3k74zbd4okmzqfbye4s6eg.py
...
#   mul_22 => mul_2
# Graph fragment:
#   %primals_6 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1" = PlaceHolder[target=primals_6]
...
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_4, %primals_5), kwargs = {})
#   %add : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_6, %mul_1), kwargs = {})
#   %mul_2 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_7, %view_3), kwargs = {})
#   %add_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%add, %mul_2), kwargs = {})
#   return %add_1
...
@triton_heuristics.pointwise(
    size_hints={'x': 67108864}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*fp32', 'in_ptr1': '*fp32', 'in_ptr2': '*bf16', 'in_ptr3': '*fp32', 'in_ptr4': '*bf16', 'out_ptr0': '*fp32', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=1, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]], (4,): [['tt.divisibility', 16]], (5,): [['tt.divisibility', 16]], (6,): [['tt.divisibility', 16]]}]},
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            triton_poi_fused__to_copy_t_0.run(primals_1, buf0, 393216, stream=stream1)
            del primals_1
            ...
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_t_0.run(primals_3, buf3, 393216, stream=stream1)
            del primals_3
            buf4 = empty_strided_cuda((128, 1024, 1024), (1048576, 1024, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear, leaky_relu, square, linear_1], Original ATen: [aten._unsafe_view, aten.leaky_relu, aten.pow, aten._to_copy]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy__unsafe_view_leaky_relu_pow_2.run(buf2, buf4, 134217728, stream=stream1)
            ...
            # Topologically Sorted Source Nodes: [linear_1, mul, add, mul_22, add_1], Original ATen: [aten._unsafe_view, aten.mul, aten.add]
            stream1 = get_raw_stream(1)
            triton_poi_fused__unsafe_view_add_mul_3.run(primals_6, primals_4, primals_5, primals_7, buf5, buf6, 50331648, stream=stream1)
            del primals_6
...
def get_args():
    ...
    primals_1 = rand_strided((1024, 384), (384, 1), device='cuda:1', dtype=torch.bfloat16)
    primals_2 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:1', dtype=torch.float32)
    primals_3 = rand_strided((384, 1024), (1024, 1), device='cuda:1', dtype=torch.bfloat16)
    ...
    primals_6 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:1', dtype=torch.float32)
