...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6o/c6oybvuj5ravcxinz7wqqa6ngj64lu5oyl72wtthkkaouyuy7xik.py
...
#   %add : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%view_1, %primals_4), kwargs = {})
#   return %add
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            _leaky_relu_sq_matmul_kernel_0.run(reinterpret_tensor(primals_2, (131072, 384), (384, 1), 0), primals_3, buf0, buf1, 24576, 1, 1, stream=stream3)
            ...
            # Topologically Sorted Source Nodes: [outs, add], Original ATen: [aten.view, aten.add]
            stream3 = get_raw_stream(3)
            triton_poi_fused_add_view_0.run(buf5, primals_4, 50331648, stream=stream3)
            del primals_4
...
def get_args():
    ...
    primals_2 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:3', dtype=torch.bfloat16)
