...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/gq/cgqwmvpvcmw3htjn2qjrtvxevmyytolau4jitjhvfqa4hakzktan.py
...
# Graph fragment:
#   %full_default_4 : Tensor "f32[131072, 128][128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([131072, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:6, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ij/cijpa5ebsikmv2xcpgpk7kq3tjrtcs52vlhg6scpfnrhpfxv7bsp.py
...
#   where => where
# Graph fragment:
#   %primals_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:6" = PlaceHolder[target=primals_1]
#   %mm_13 : Tensor "bf16[131072, 128][128, 1]cuda:6" = PlaceHolder[target=mm_13]
#   %index_put : Tensor "f32[131072, 128][128, 1]cuda:6" = PlaceHolder[target=index_put]
...
#   %mul_42 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.3535533905932738), kwargs = {})
...
#   %expand : Tensor "f32[128, 1024, 8, 128][131072, 128, 0, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_4, [128, 1024, 8, 128]), kwargs = {})
...
#   %eq : Tensor "b8[128, 1024, 8][8192, 8, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%primals_1, -1), kwargs = {})
#   %unsqueeze_5 : Tensor "b8[128, 1024, 8, 1][8192, 8, 1, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
...
#   %where : Tensor "f32[128, 1024, 8, 128][1048576, 1024, 128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_5, %full_default_3, %convert_element_type_default), kwargs = {})
#   %full_default_4 : Tensor "f32[131072, 128][128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([131072, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:6, pin_memory: False})
#   %index_put : Tensor "f32[131072, 128][128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_4, [%primals_1], %where, True), kwargs = {})
#   return %buf44
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/uj/cujx7s7wbcrkweuybgyn62kpruvfxs7much3anq6aq46avq4kl4g.py
...
# Graph fragment:
#   %buf44 : Tensor "f32[131072, 128][128, 1]cuda:6" = PlaceHolder[target=buf44]
#   %convert_element_type_80 : Tensor "bf16[131072, 128][128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(6):
            ...
            # Topologically Sorted Source Nodes: [full_default_4], Original ATen: [aten.embedding_dense_backward]
            stream6 = get_raw_stream(6)
            triton_poi_fused_embedding_dense_backward_12.run(buf43, 16777216, stream=stream6)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_42, unsqueeze_4, expand, convert_element_type_default, eq, unsqueeze_5, full_default_3, where, full_default_4, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream6 = get_raw_stream(6)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_13.run(primals_1, buf42, buf43, 1048576, 128, stream=stream6)
            del primals_1
            buf45 = buf42; del buf42  # reuse
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream6 = get_raw_stream(6)
            triton_poi_fused_embedding_dense_backward_14.run(buf43, buf45, 16777216, stream=stream6)
            del buf43
...
def get_args():
    ...
    primals_1 = rand_strided((128, 1024, 8), (8192, 8, 1), device='cuda:6', dtype=torch.int64)
