...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/4b/c4bt7ou2dae6luxzuwmjf222ki2f2n75vx2oeylgk4sxctlnncla.py
...
# Graph fragment:
#   %full_default_4 : Tensor "f32[131072, 96][96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([131072, 96], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:3, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/63/c63y7k35eaqdaz6wlkqelyi55lkck33p5i5jlxdzx7hc6w7u7nrl.py
...
#   %mm_13 : Tensor "bf16[131072, 96][96, 1]cuda:3" = PlaceHolder[target=mm_13]
#   %index_put : Tensor "f32[131072, 96][96, 1]cuda:3" = PlaceHolder[target=index_put]
#   %view_30 : Tensor "bf16[128, 1024, 96][98304, 96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm_13, [128, 1024, 96]), kwargs = {})
#   %convert_element_type_75 : Tensor "f32[128, 1024, 96][98304, 96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_30, torch.float32), kwargs = {})
#   %mul_42 : Tensor "f32[128, 1024, 96][98304, 96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.3535533905932738), kwargs = {})
#   %unsqueeze_4 : Tensor "f32[128, 1024, 1, 96][98304, 96, 96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%mul_42, -2), kwargs = {})
#   %expand : Tensor "f32[128, 1024, 8, 96][98304, 96, 0, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_4, [128, 1024, 8, 96]), kwargs = {})
#   %convert_element_type_default : Tensor "f32[128, 1024, 8, 96][786432, 768, 96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%expand, torch.float32), kwargs = {})
...
#   %where : Tensor "f32[128, 1024, 8, 96][786432, 768, 96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_5, %full_default_3, %convert_element_type_default), kwargs = {})
#   %full_default_4 : Tensor "f32[131072, 96][96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([131072, 96], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:3, pin_memory: False})
#   %index_put : Tensor "f32[131072, 96][96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_4, [%primals_1], %where, True), kwargs = {})
#   return %buf44
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/zy/czymu56tbddwi5ky7aq6sws65hqwhztltk7krfpwkx35dilvsdkk.py
...
# Graph fragment:
#   %buf44 : Tensor "f32[131072, 96][96, 1]cuda:3" = PlaceHolder[target=buf44]
#   %convert_element_type_80 : Tensor "bf16[131072, 96][96, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            # Topologically Sorted Source Nodes: [full_default_4], Original ATen: [aten.embedding_dense_backward]
            stream3 = get_raw_stream(3)
            triton_poi_fused_embedding_dense_backward_12.run(buf43, 12582912, stream=stream3)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_42, unsqueeze_4, expand, convert_element_type_default, eq, unsqueeze_5, full_default_3, where, full_default_4, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream3 = get_raw_stream(3)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_13.run(primals_1, buf42, buf43, 1048576, 96, stream=stream3)
            del primals_1
            buf45 = buf42; del buf42  # reuse
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream3 = get_raw_stream(3)
            triton_poi_fused_embedding_dense_backward_14.run(buf43, buf45, 12582912, stream=stream3)
            del buf43
...
def get_args():
    ...
    view = rand_strided((131072, 96), (96, 1), device='cuda:3', dtype=torch.bfloat16)
    ...
    permute_34 = rand_strided((384, 96), (96, 1), device='cuda:3', dtype=torch.bfloat16)
