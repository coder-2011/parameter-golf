...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/36/c36gmu6r4vmjkjsojdw3h4b5acjuokzhfd2ivym6az7xmuvmj2nq.py
...
# Graph fragment:
#   %full_default_5 : Tensor "f32[65536, 128][128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([65536, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:7, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/qv/cqvt27cpdl4kisgfeblzycwkxk6kyr5anf65hcu2dg7zunga4rls.py
...
#   %index_put : Tensor "f32[65536, 128][128, 1]cuda:7" = PlaceHolder[target=index_put]
...
#   %mul_45 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.3535533905932738), kwargs = {})
...
#   %expand : Tensor "f32[128, 1024, 8, 128][131072, 128, 0, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_6, [128, 1024, 8, 128]), kwargs = {})
#   %convert_element_type_default : Tensor "f32[128, 1024, 8, 128][1048576, 1024, 128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%expand, torch.float32), kwargs = {})
...
#   %full_default_4 : Tensor "f32[][]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.full.default](args = ([], 0.0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:7, pin_memory: False})
#   %where : Tensor "f32[128, 1024, 8, 128][1048576, 1024, 128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_7, %full_default_4, %convert_element_type_default), kwargs = {})
#   %full_default_5 : Tensor "f32[65536, 128][128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([65536, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:7, pin_memory: False})
#   %index_put : Tensor "f32[65536, 128][128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_5, [%add_1], %where, True), kwargs = {})
#   return %buf43
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/wd/cwdqy7fl3q54huy3um3n4hzsc5ghqfnnllbao5oqhu4yxw5tg6jr.py
...
# Graph fragment:
#   %buf43 : Tensor "f32[65536, 128][128, 1]cuda:7" = PlaceHolder[target=buf43]
#   %convert_element_type_80 : Tensor "bf16[65536, 128][128, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(7):
            ...
            # Topologically Sorted Source Nodes: [full_default_5], Original ATen: [aten.embedding_dense_backward]
            stream7 = get_raw_stream(7)
            triton_poi_fused_embedding_dense_backward_13.run(buf42, 8388608, stream=stream7)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_45, unsqueeze_6, expand, convert_element_type_default, eq, unsqueeze_7, full_default_4, where, full_default_5, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream7 = get_raw_stream(7)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_14.run(add_1, buf41, buf42, 1048576, 128, stream=stream7)
            del add_1
            del buf41
            buf44 = empty_strided_cuda((65536, 128), (128, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream7 = get_raw_stream(7)
            triton_poi_fused_embedding_dense_backward_15.run(buf42, buf44, 8388608, stream=stream7)
            del buf42
