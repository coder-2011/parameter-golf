...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/nv/cnviv6ar45nz7so3kum3rfaoruu2puuf2xs4vbrhej623c3fiide.py
# Topologically Sorted Source Nodes: [zeros], Original ATen: [aten.zeros]
# Source node to ATen node mapping:
#   zeros => full_default
# Graph fragment:
#   %full_default : Tensor "bf16[350, 1000, 384][384000, 384, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([350, 1000, 384], 0), kwargs = {dtype: torch.bfloat16, layout: torch.strided, device: cuda:6, pin_memory: False})
#   return %full_default
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(6):
            ...
            triton_poi_fused_zeros_0.run(buf0, 134400000, stream=stream6)
        return (buf0, )
