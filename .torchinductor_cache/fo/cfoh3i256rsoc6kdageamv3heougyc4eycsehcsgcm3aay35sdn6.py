...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/xk/cxkqvl5dsgpxbn4bprncctt2cwww7cju2py7upvdksellz52agsj.py
...
#   %primals_7 : Tensor "bf16[65536, 120][120, 1]cuda:1" = PlaceHolder[target=primals_7]
#   %sum_1 : Tensor "f32[128, 1024, 120][122880, 120, 1]cuda:1" = PlaceHolder[target=sum_1]
#   %embedding_1 : Tensor "bf16[128, 1024, 8, 120][983040, 960, 120, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_7, %add_1), kwargs = {})
#   %sum_1 : Tensor "f32[128, 1024, 120][122880, 120, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%embedding_1, [-2]), kwargs = {dtype: torch.float32})
#   %mul_3 : Tensor "f32[128, 1024, 120][122880, 120, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 120][122880, 120, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_3, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/vu/cvuzfvi7qcg6fgfrfxpjn6ode2hi3bkao754pitrphbkp5o2wjfd.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
# Source node to ATen node mapping:
#   linear => convert_element_type_default_7, permute
# Graph fragment:
#   %primals_8 : Tensor "bf16[384, 120][120, 1]cuda:1" = PlaceHolder[target=primals_8]
#   %convert_element_type_default_7 : Tensor "bf16[384, 120][120, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_8, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[120, 384][1, 120]cuda:1"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_7, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/74/c74tcz4e3hmzrijujxml2jufthgyrdyqgor26n3fhdf5vfawjxor.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/re/cret3jphvk4dm3qq6tnmavb7mgkybmv3xdsfnsfhbav7hdpzx7k5.py
...
#   %buf13 : Tensor "f32[128, 1024, 1][1024, 1, 131072]cuda:1" = PlaceHolder[target=buf13]
...
#   %view_1 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm, [128, 1024, 384]), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/tc/ctcp3cj4y466cvtt5j6jrmr47bal7zl3ysxgf45ktrinrp726udr.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/jb/cjbkdtpoo7fqcgsnxje32rolsqqko4lbqcihiynvm3an2ymvfmqc.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/rv/crvq5s3kwppqirlanns25oqvdmhhzqb6xrwzxl2257dlasep2zow.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/wa/cwa4d4fnqztfv34n2e6conhu6hwjhw3dduu6ahz2zkttrlj7j7sb.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/5i/c5i2lw4x3cjkodsafvtb2e3iypo6y2omafrglxs3vjdermsomsmj.py
...
#   %buf34 : Tensor "f32[128, 1024, 1][1024, 1, 131072]cuda:1" = PlaceHolder[target=buf34]
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            # Topologically Sorted Source Nodes: [embedding_1, sum_1, mul_3, linear], Original ATen: [aten.embedding, aten.sum, aten.mul, aten._to_copy]
            stream1 = get_raw_stream(1)
            triton_per_fused__to_copy_embedding_mul_sum_3.run(buf3, primals_7, buf6, 15728640, 8, stream=stream1)
            del primals_7
            buf5 = empty_strided_cuda((120, 384), (1, 120), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_t_4.run(primals_8, buf5, 46080, stream=stream1)
            del primals_8
            ...
            # Topologically Sorted Source Nodes: [to_2], Original ATen: [aten._to_copy]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_5.run(primals_9, buf8, 384, stream=stream1)
            del primals_9
            ...
            # Topologically Sorted Source Nodes: [linear, linear_1, sigmoid, mul_4, to_5, mul_5, add_2, to_6, sigmoid_1, getitem_8, zeros_like, getitem_10, cat, ne, to_7, unsqueeze, mul_6, sub, mul_7, mul_8, add_3, to_8, rms_norm], Original ATen: [aten._unsafe_view, aten.view, aten.sigmoid, aten.mul, aten._to_copy, aten.add, aten.unsqueeze, aten.zeros_like, aten.slice, aten.cat, aten.ne, aten.rsub, aten._fused_rms_norm]
            stream1 = get_raw_stream(1)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_cat_mul_ne_rsub_sigmoid_slice_unsqueeze_view_zeros_like_6.run(buf14, primals_5, buf0, buf7, buf9, primals_11, primals_12, primals_13, buf11, buf10, buf12, buf15, 131072, 384, stream=stream1)
            ...
            stream1 = get_raw_stream(1)
            _fused_qkv_postprocess_fwd_kernel_0.run(reinterpret_tensor(buf16, (128, 1024, 16, 48), (786432, 768, 48, 1), 0), buf17, buf18, reinterpret_tensor(primals_1, (1024, 24), (24, 1), 0), reinterpret_tensor(primals_2, (1024, 24), (24, 1), 0), 1572864, 1, 1, stream=stream1)
            buf21 = empty_strided_cuda((128, 1024, 4, 48), (196608, 192, 48, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear_2, view, getitem, v], Original ATen: [aten._unsafe_view, aten.view, aten.slice, aten.clone]
            stream1 = get_raw_stream(1)
            triton_poi_fused__unsafe_view_clone_slice_view_7.run(buf16, buf21, 25165824, stream=stream1)
            ...
            # Topologically Sorted Source Nodes: [to_10, linear_3], Original ATen: [aten._to_copy, aten.t]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_t_8.run(primals_15, buf28, 96, stream=stream1)
            del primals_15
            buf29 = empty_strided_cuda((128, 1024, 12), (12288, 12, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [getitem_14, contiguous_3], Original ATen: [aten.slice, aten.clone]
            stream1 = get_raw_stream(1)
            triton_poi_fused_clone_slice_9.run(buf15, buf29, 1572864, stream=stream1)
            ...
            # Topologically Sorted Source Nodes: [transpose_3, linear_3, mul_9, sigmoid_2, getitem_15, mul_10], Original ATen: [aten.transpose, aten._unsafe_view, aten.mul, aten.sigmoid, aten.unsqueeze]
            stream1 = get_raw_stream(1)
            triton_poi_fused__unsafe_view_mul_sigmoid_transpose_unsqueeze_10.run(buf23, buf30, buf31, 50331648, stream=stream1)
            ...
            # Topologically Sorted Source Nodes: [linear_4, add_4, to_12, rms_norm_1], Original ATen: [aten._unsafe_view, aten.add, aten._to_copy, aten._fused_rms_norm]
            stream1 = get_raw_stream(1)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_11.run(buf33, buf35, buf12, primals_17, buf36, 131072, 384, stream=stream1)
...
def get_args():
    ...
    primals_7 = rand_strided((65536, 120), (120, 1), device='cuda:1', dtype=torch.bfloat16)
    primals_8 = rand_strided((384, 120), (120, 1), device='cuda:1', dtype=torch.bfloat16)
