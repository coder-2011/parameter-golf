...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/qx/cqxovfplc3rsnrnr4l3n2kjltjmmbfda2zn6ahd6unkwl34dm5do.py
# Topologically Sorted Source Nodes: [zeros], Original ATen: [aten.zeros]
# Source node to ATen node mapping:
#   zeros => full_default
# Graph fragment:
#   %full_default : Tensor "bf16[512, 1000, 384][384000, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([512, 1000, 384], 0), kwargs = {dtype: torch.bfloat16, layout: torch.strided, device: cuda:5, pin_memory: False})
#   return %full_default
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(5):
            ...
            triton_poi_fused_zeros_0.run(buf0, 196608000, stream=stream5)
        return (buf0, )
