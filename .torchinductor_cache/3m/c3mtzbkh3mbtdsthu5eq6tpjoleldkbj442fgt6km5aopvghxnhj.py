...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ss/cssssh5y74u7ql77o4ciooadc24m6iar7g2pusos2gvbhxnvrpnh.py
...
# Graph fragment:
#   %primals_5 : Tensor "i64[128, 1024][1024, 1]cuda:3" = PlaceHolder[target=primals_5]
#   %slice_scatter_default : Tensor "i64[128, 1024, 4][4096, 4, 1]cuda:3" = PlaceHolder[target=slice_scatter_default]
...
#   %select : Tensor "i64[128, 4][4096, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%empty, 1, 0), kwargs = {})
#   %copy : Tensor "i64[128, 4][4096, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%select, %full_default), kwargs = {})
#   %select_scatter_default : Tensor "i64[128, 1024, 4][4096, 4, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.select_scatter.default](args = (%empty, %copy, 1, 0), kwargs = {})
...
#   %mul : Tensor "i64[128, 1023, 4][4092, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %lift_fresh_copy), kwargs = {})
...
#   %mul_1 : Tensor "i64[128, 1023, 4][4092, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze_1, %lift_fresh_copy_1), kwargs = {})
#   %bitwise_xor : Tensor "i64[128, 1023, 4][4092, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.bitwise_xor.Tensor](args = (%mul, %mul_1), kwargs = {})
#   %remainder : Tensor "i64[128, 1023, 4][4092, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 8191), kwargs = {})
#   %add : Tensor "i64[128, 1023, 4][4092, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%remainder, 1), kwargs = {})
#   %slice_4 : Tensor "i64[128, 1023, 4][4096, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%select_scatter_default, 1, 1, 9223372036854775807), kwargs = {})
#   %copy_1 : Tensor "i64[128, 1023, 4][4096, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%slice_4, %add), kwargs = {})
#   %slice_scatter_default : Tensor "i64[128, 1024, 4][4096, 4, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.slice_scatter.default](args = (%select_scatter_default, %copy_1, 1, 1, 9223372036854775807), kwargs = {})
...
#   %add_1 : Tensor "i64[128, 1024, 4][4096, 4, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
...
@triton_heuristics.pointwise(
    size_hints={'x': 524288}, 
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*i64', 'in_ptr0': '*i64', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=3, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]]}]},
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/fg/cfg4pgaonzjsglmuuekqgsnkxrdqno2ct2t4ft4mk6zwaamhr3tj.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
# Source node to ATen node mapping:
#   linear => convert_element_type_default_7, permute
# Graph fragment:
#   %primals_8 : Tensor "bf16[384, 128][128, 1]cuda:3" = PlaceHolder[target=primals_8]
#   %convert_element_type_default_7 : Tensor "bf16[384, 128][128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_8, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[128, 384][1, 128]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_7, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/7p/c7pl45xjqie7kc2urfd7k32prn6ggdm3q2hphylfxcxanqj2gqht.py
...
#   %primals_7 : Tensor "bf16[32768, 128][128, 1]cuda:3" = PlaceHolder[target=primals_7]
#   %embedding_1 : Tensor "bf16[128, 1024, 4, 128][524288, 512, 128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_7, %add_1), kwargs = {})
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%embedding_1, [-2]), kwargs = {dtype: torch.float32})
#   %mul_3 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.5), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 128][131072, 128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_3, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/mq/cmqvsxzisozyatw4hrewkjtfleb7leydnvhdyuhrxoescq2lnrox.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6m/c6medklguswzvbxfhn72ehp7rmhbjk3iiabmtb2gbejkswyd43ks.py
...
#   %buf12 : Tensor "f32[128, 1024, 1][1024, 1, 131072]cuda:3" = PlaceHolder[target=buf12]
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/3a/c3afs4q3rnq6o2u3ejo7belf75qhyp6lt5in6lmkb3gpusqdxpy7.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/mt/cmths3ecuakj7kdkqrocn6r67d2gbvvalaue2u2teh3waz2tjldh.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ny/cny77s2m5jbajkibkveqy3slyzpsfmezxrhjqql3ltjp6nwhawm6.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/f3/cf3bowoq3fivltluzilhhne7wrxr5hxw343bvnzwk7n444lwfyiv.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/7h/c7hysc5qvsfl35h4fjhzoyzibxlghwjrzydxaqehj4xoekhl24h2.py
...
#   %buf33 : Tensor "f32[128, 1024, 1][1024, 1, 131072]cuda:3" = PlaceHolder[target=buf33]
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            # Topologically Sorted Source Nodes: [getitem_4, getitem_5, tensor, tensor_1, setitem, getitem_6, mul, getitem_7, mul_1, bitwise_xor, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.slice, aten.lift_fresh, aten.select, aten.fill, aten.unsqueeze, aten.mul, aten.bitwise_xor, aten.remainder, aten.add, aten.copy, aten.arange]
            stream3 = get_raw_stream(3)
            triton_poi_fused_add_arange_bitwise_xor_copy_fill_lift_fresh_mul_remainder_select_slice_unsqueeze_1.run(buf3, primals_5, 524288, stream=stream3)
            buf4 = empty_strided_cuda((128, 384), (1, 128), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
            stream3 = get_raw_stream(3)
            triton_poi_fused__to_copy_t_2.run(primals_8, buf4, 49152, stream=stream3)
            del primals_8
            buf5 = empty_strided_cuda((128, 1024, 128), (131072, 128, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding_1, sum_1, mul_3, linear], Original ATen: [aten.embedding, aten.sum, aten.mul, aten._to_copy]
            stream3 = get_raw_stream(3)
            triton_poi_fused__to_copy_embedding_mul_sum_3.run(buf3, primals_7, buf5, 16777216, stream=stream3)
            del primals_7
            ...
            # Topologically Sorted Source Nodes: [to_2], Original ATen: [aten._to_copy]
            stream3 = get_raw_stream(3)
            triton_poi_fused__to_copy_4.run(primals_9, buf7, 384, stream=stream3)
            del primals_9
            ...
            # Topologically Sorted Source Nodes: [linear, linear_1, sigmoid, mul_4, to_5, mul_5, add_2, to_6, sigmoid_1, getitem_8, zeros_like, getitem_10, cat, ne, to_7, unsqueeze, mul_6, sub, mul_7, mul_8, add_3, to_8, rms_norm], Original ATen: [aten._unsafe_view, aten.view, aten.sigmoid, aten.mul, aten._to_copy, aten.add, aten.unsqueeze, aten.zeros_like, aten.slice, aten.cat, aten.ne, aten.rsub, aten._fused_rms_norm]
            stream3 = get_raw_stream(3)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_cat_mul_ne_rsub_sigmoid_slice_unsqueeze_view_zeros_like_5.run(buf13, primals_5, buf0, buf6, buf8, primals_11, primals_12, primals_13, buf10, buf9, buf11, buf14, 131072, 384, stream=stream3)
            ...
            stream3 = get_raw_stream(3)
            _fused_qkv_postprocess_fwd_kernel_0.run(reinterpret_tensor(buf15, (128, 1024, 16, 48), (786432, 768, 48, 1), 0), buf16, buf17, reinterpret_tensor(primals_1, (1024, 24), (24, 1), 0), reinterpret_tensor(primals_2, (1024, 24), (24, 1), 0), 1572864, 1, 1, stream=stream3)
            buf20 = empty_strided_cuda((128, 1024, 4, 48), (196608, 192, 48, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear_2, view, getitem, v], Original ATen: [aten._unsafe_view, aten.view, aten.slice, aten.clone]
            stream3 = get_raw_stream(3)
            triton_poi_fused__unsafe_view_clone_slice_view_6.run(buf15, buf20, 25165824, stream=stream3)
            ...
            # Topologically Sorted Source Nodes: [to_10, linear_3], Original ATen: [aten._to_copy, aten.t]
            stream3 = get_raw_stream(3)
            triton_poi_fused__to_copy_t_7.run(primals_15, buf27, 96, stream=stream3)
            del primals_15
            buf28 = empty_strided_cuda((128, 1024, 12), (12288, 12, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [getitem_14, contiguous_3], Original ATen: [aten.slice, aten.clone]
            stream3 = get_raw_stream(3)
            triton_poi_fused_clone_slice_8.run(buf14, buf28, 1572864, stream=stream3)
            ...
            # Topologically Sorted Source Nodes: [transpose_3, linear_3, mul_9, sigmoid_2, getitem_15, mul_10], Original ATen: [aten.transpose, aten._unsafe_view, aten.mul, aten.sigmoid, aten.unsqueeze]
            stream3 = get_raw_stream(3)
            triton_poi_fused__unsafe_view_mul_sigmoid_transpose_unsqueeze_9.run(buf22, buf29, buf30, 50331648, stream=stream3)
            ...
            # Topologically Sorted Source Nodes: [linear_4, add_4, to_12, rms_norm_1], Original ATen: [aten._unsafe_view, aten.add, aten._to_copy, aten._fused_rms_norm]
            stream3 = get_raw_stream(3)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_10.run(buf32, buf34, buf11, primals_17, buf35, 131072, 384, stream=stream3)
...
def get_args():
    ...
    primals_7 = rand_strided((32768, 128), (128, 1), device='cuda:3', dtype=torch.bfloat16)
    primals_8 = rand_strided((384, 128), (128, 1), device='cuda:3', dtype=torch.bfloat16)
