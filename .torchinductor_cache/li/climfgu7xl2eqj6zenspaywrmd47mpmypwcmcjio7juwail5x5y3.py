...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/3n/c3njdkss43itjwdh62w7pvr7o4u2hqoesaam5o2aoi2qj4u6pn7p.py
...
#   %primals_4 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:7" = PlaceHolder[target=primals_4]
...
#   %mul_3 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tangents_1, %view_1), kwargs = {})
#   %sum_1 : Tensor "f32[1, 1, 384][384, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%mul_3, [0, 1], True), kwargs = {dtype: torch.float32})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/4w/c4wyhryse5xbeqjj6cb7hagibjavdc2xuy3zql2sf65ne5jr5tnz.py
...
# Graph fragment:
#   %tangents_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:7" = PlaceHolder[target=tangents_1]
#   %primals_3 : Tensor "f32[1, 1, 384][384, 384, 1]cuda:7" = PlaceHolder[target=primals_3]
#   %primals_6 : Tensor "f32[1, 1, 384][384, 384, 1]cuda:7" = PlaceHolder[target=primals_6]
#   %mul_2 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tangents_1, %primals_6), kwargs = {})
#   %convert_element_type_5 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_2, torch.bfloat16), kwargs = {})
#   %mul_4 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tangents_1, %primals_3), kwargs = {})
#   %convert_element_type_6 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_4, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/gd/cgduuvwwziebfativ6mo6umbtze4oavespyteihrty5tp4btzmd6.py
...
#   view_3 => view_3
# Graph fragment:
#   %mm_2 : Tensor "bf16[131072, 1536][1536, 1]cuda:7" = PlaceHolder[target=mm_2]
#   %view_3 : Tensor "bf16[128, 1024, 1536][1572864, 1536, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm_2, [128, 1024, 1536]), kwargs = {})
#   %convert_element_type_11 : Tensor "f32[128, 1024, 1536][1572864, 1536, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_3, torch.float32), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(7):
            ...
            triton_red_fused__unsafe_view_mul_sum_0.run(tangents_1, mm, primals_4, buf0, buf2, 134016, 376, stream=stream7)
            del mm
            del primals_4
            ...
            # Topologically Sorted Source Nodes: [mul_2, convert_element_type_5, mul_4, convert_element_type_6], Original ATen: [aten.mul, aten._to_copy]
            stream7 = get_raw_stream(7)
            triton_poi_fused__to_copy_mul_2.run(tangents_1, primals_3, primals_6, buf4, buf5, 50331648, stream=stream7)
            del primals_3
            del primals_6
            ...
            # Topologically Sorted Source Nodes: [view_3, convert_element_type_11], Original ATen: [aten.view, aten._to_copy]
            stream7 = get_raw_stream(7)
            triton_poi_fused__to_copy_view_3.run(buf7, buf8, 201326592, stream=stream7)
            del buf7
...
def get_args():
    ...
    primals_3 = rand_strided((1, 1, 384), (384, 384, 1), device='cuda:7', dtype=torch.float32)
    primals_4 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:7', dtype=torch.bfloat16)
    primals_6 = rand_strided((1, 1, 384), (384, 384, 1), device='cuda:7', dtype=torch.float32)
    view = rand_strided((131072, 1536), (1536, 1), device='cuda:7', dtype=torch.bfloat16)
    mm = rand_strided((131072, 384), (384, 1), device='cuda:7', dtype=torch.bfloat16)
    permute_3 = rand_strided((384, 1536), (1536, 1), device='cuda:7', dtype=torch.bfloat16)
