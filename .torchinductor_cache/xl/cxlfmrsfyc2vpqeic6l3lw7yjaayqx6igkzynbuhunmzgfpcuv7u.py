...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/rx/crxmyuri4plfmws3opyyuabrh5fsmakwx2wawy3hoiu4tkvvck23.py
...
#   %convert_element_type_default_1 : Tensor "bf16[384, 1536][1536, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_1, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[1536, 384][1, 1536]cuda:1"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_1, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/it/cit7znu42mek5fmkvdx57stdf7bxhiabedre2vbzg2kolfrkho2j.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
# Source node to ATen node mapping:
#   linear => convert_element_type_2
# Graph fragment:
#   %primals_2 : Tensor "f32[128, 1024, 1536][1572864, 1536, 1]cuda:1" = PlaceHolder[target=primals_2]
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 1536][1572864, 1536, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_2, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/us/cusslmmmll6lg7yug5kacqkjnyupazpb5l3iwpn355kshim5ch67.py
...
#   mul_1 => mul_1
# Graph fragment:
#   %primals_5 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1" = PlaceHolder[target=primals_5]
...
#   %mul : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_3, %primals_4), kwargs = {})
#   %add : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_5, %mul), kwargs = {})
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_6, %view_1), kwargs = {})
#   %add_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%add, %mul_1), kwargs = {})
#   return %add_1
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            triton_poi_fused__to_copy_t_0.run(primals_1, buf0, 589824, stream=stream1)
            del primals_1
            buf1 = empty_strided_cuda((128, 1024, 1536), (1572864, 1536, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_1.run(primals_2, buf1, 201326592, stream=stream1)
            del primals_2
            ...
            # Topologically Sorted Source Nodes: [linear, mul, add, mul_1, add_1], Original ATen: [aten._unsafe_view, aten.mul, aten.add]
            stream1 = get_raw_stream(1)
            triton_poi_fused__unsafe_view_add_mul_2.run(primals_5, primals_3, primals_4, primals_6, buf2, buf3, 50331648, stream=stream1)
            del primals_5
...
def get_args():
    ...
    primals_2 = rand_strided((128, 1024, 1536), (1572864, 1536, 1), device='cuda:1', dtype=torch.float32)
    ...
    primals_5 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:1', dtype=torch.float32)
