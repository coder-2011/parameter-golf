...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/zm/czm7d6qxkzt6yry5y2v2hqqhugicbvbpz6vyd7fpemqxlmijnde7.py
# Topologically Sorted Source Nodes: [zeros], Original ATen: [aten.zeros]
# Source node to ATen node mapping:
#   zeros => full_default
# Graph fragment:
#   %full_default : Tensor "bf16[349, 1000, 384][384000, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([349, 1000, 384], 0), kwargs = {dtype: torch.bfloat16, layout: torch.strided, device: cuda:2, pin_memory: False})
#   return %full_default
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_zeros_0.run(buf0, 134016000, stream=stream2)
        return (buf0, )
