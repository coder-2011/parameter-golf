...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/66/c66ep3aoqqirasqx57vmjdtfnnm5fm56xtbtxd5v6gl43oq4j4ov.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/tg/ctgaenqc2ackwepuzlmrzqefvvycbzyd7elepxrg2y4bx3xgidai.py
...
#   %remainder : Tensor "i64[128, 1023, 8][8184, 8, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
...
#   %add_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/zk/czkedrqp2fotoh2tucivwid4rluf3s7ojczrbbghpjnv6666x2mq.py
...
#   %embedding : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_6, %primals_5), kwargs = {})
#   return %embedding
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(6):
            ...
            triton_poi_fused_bitwise_xor_lift_fresh_mul_slice_unsqueeze_0.run(primals_5, buf2, 130944, 8, stream=stream6)
            buf3 = empty_strided_cuda((128, 1024, 8), (8192, 8, 1), torch.int64)
            # Topologically Sorted Source Nodes: [setitem, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.lift_fresh, aten.select, aten.fill, aten.remainder, aten.add, aten.slice, aten.copy, aten.arange, aten.mul]
            stream6 = get_raw_stream(6)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_1.run(buf2, buf3, 1048576, stream=stream6)
            del buf2
            buf0 = empty_strided_cuda((128, 1024, 384), (393216, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream6 = get_raw_stream(6)
            triton_poi_fused_embedding_2.run(primals_5, primals_6, buf0, 131072, 384, stream=stream6)
            del primals_6
