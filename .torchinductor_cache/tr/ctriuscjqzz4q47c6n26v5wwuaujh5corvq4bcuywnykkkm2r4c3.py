...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6m/c6mkzezr2q3fyh5tnjcgf3jkqxbqqhnlerou2a75o5ykvk5rqfgn.py
...
#   %primals_7 : Tensor "bf16[65536, 128][128, 1]cuda:3" = PlaceHolder[target=primals_7]
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:3" = PlaceHolder[target=sum_1]
#   %embedding_1 : Tensor "bf16[128, 1024, 8, 128][1048576, 1024, 128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_7, %add_1), kwargs = {})
...
#   %mul_3 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/dl/cdlxox3d2z3iokkxr26v3cby52hls2iffuoocd3r3biarkz4oylc.py
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            # Topologically Sorted Source Nodes: [embedding_1, sum_1, mul_3, linear], Original ATen: [aten.embedding, aten.sum, aten.mul, aten._to_copy]
            stream3 = get_raw_stream(3)
            triton_per_fused__to_copy_embedding_mul_sum_3.run(buf3, primals_7, buf6, 16777216, 8, stream=stream3)
            del primals_7
            buf5 = empty_strided_cuda((128, 384), (1, 128), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
            stream3 = get_raw_stream(3)
            triton_poi_fused__to_copy_t_4.run(primals_8, buf5, 49152, stream=stream3)
            del primals_8
...
def get_args():
    ...
    primals_7 = rand_strided((65536, 128), (128, 1), device='cuda:3', dtype=torch.bfloat16)
