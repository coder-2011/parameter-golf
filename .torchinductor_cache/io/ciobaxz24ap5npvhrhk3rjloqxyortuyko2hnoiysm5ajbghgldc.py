...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6o/c6oxq7oznpbbvz6labg6g237fcgawza7apwumu5lw35qffcow5wg.py
...
#   %remainder : Tensor "i64[349, 999, 8][7992, 8, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
...
#   %mul_2 : Tensor "i64[8][1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%iota, 16384), kwargs = {})
...
#   %embedding : Tensor "bf16[349, 1000, 384][384000, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%arg5_1, %arg4_1), kwargs = {})
#   return %embedding
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(5):
            ...
            # Topologically Sorted Source Nodes: [setitem, setitem_1, remainder, add, arange, mul_2, add_1], Original ATen: [aten.select, aten.lift_fresh, aten.fill, aten.slice, aten.remainder, aten.add, aten.copy, aten.arange, aten.mul]
            stream5 = get_raw_stream(5)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_1.run(buf1, buf2, 2792000, stream=stream5)
            del buf1
            buf3 = empty_strided_cuda((349, 1000, 384), (384000, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream5 = get_raw_stream(5)
            triton_poi_fused_embedding_2.run(arg4_1, arg5_1, buf3, 134016000, stream=stream5)
            del arg4_1
            del arg5_1
