...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/q5/cq5fovu2choyc4227gf7pv3eh3wpyjvdykxx4ktub3hmp7f6jjsr.py
...
# Graph fragment:
#   %full_default_4 : Tensor "f32[196608, 128][128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([196608, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:3, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/t3/ct3ypdmulp4fviy3o7f6qgaecsusoiymzya5aa2bnslupcby2gkm.py
...
#   %index_put : Tensor "f32[196608, 128][128, 1]cuda:3" = PlaceHolder[target=index_put]
...
#   %full_default_4 : Tensor "f32[196608, 128][128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([196608, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:3, pin_memory: False})
#   %index_put : Tensor "f32[196608, 128][128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_4, [%primals_1], %where, True), kwargs = {})
#   return %buf44
...
def triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_13(in_ptr0, in_ptr1, out_ptr0, ynumel, xnumel, YBLOCK : tl.constexpr, XBLOCK : tl.constexpr):
    ...
    tl.device_assert(((0 <= tmp4) & (tmp4 < 196608)) | ~(ymask), "index out of bounds: 0 <= tmp4 < 196608")
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/zv/czvsne6tqv2wrgzrrfopcahkofk6scax7flfp4nnor6mwkggeuht.py
...
# Graph fragment:
#   %buf44 : Tensor "f32[196608, 128][128, 1]cuda:3" = PlaceHolder[target=buf44]
#   %convert_element_type_80 : Tensor "bf16[196608, 128][128, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            del buf40
            del permute_34
            buf43 = empty_strided_cuda((196608, 128), (128, 1), torch.float32)
            # Topologically Sorted Source Nodes: [full_default_4], Original ATen: [aten.embedding_dense_backward]
            stream3 = get_raw_stream(3)
            triton_poi_fused_embedding_dense_backward_12.run(buf43, 25165824, stream=stream3)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_42, unsqueeze_4, expand, convert_element_type_default, eq, unsqueeze_5, full_default_3, where, full_default_4, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream3 = get_raw_stream(3)
            ...
            del buf42
            del primals_1
            buf45 = reinterpret_tensor(buf15, (196608, 128), (128, 1), 0); del buf15  # reuse
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream3 = get_raw_stream(3)
            triton_poi_fused_embedding_dense_backward_14.run(buf43, buf45, 25165824, stream=stream3)
            del buf43
