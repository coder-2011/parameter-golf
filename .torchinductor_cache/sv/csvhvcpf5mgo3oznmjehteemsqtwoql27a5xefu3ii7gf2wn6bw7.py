...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/gz/cgzs7niyld7nw6invdoxbhllhgr2a6qsvzztkvpua4ffdxc2ac6h.py
...
# Graph fragment:
#   %inductor_seeds_default : Tensor "i64[1][1]cuda:3" = PlaceHolder[target=inductor_seeds_default]
#   %inductor_random_default : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:3" = PlaceHolder[target=inductor_random_default]
#   %inductor_lookup_seed_default : Tensor "i64[][]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.inductor_lookup_seed.default](args = (%inductor_seeds_default, 0), kwargs = {})
#   %inductor_random_default : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.inductor_random.default](args = ([128, 1024, 384], %inductor_lookup_seed_default, randn), kwargs = {})
#   %convert_element_type_default : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%inductor_random_default, torch.bfloat16), kwargs = {})
...
@triton_heuristics.pointwise(
    ...
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i64', 'out_ptr1': '*bf16', 'load_seed_offset': 'i32', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=3, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]]}]},
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/xw/cxw7envzmbebfxhpygz76ckwkxgr2kt4r6luramru57vlv53cxxv.py
...
#   mul_ => mul
# Graph fragment:
#   %uniform : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3" = PlaceHolder[target=uniform]
#   %erfinv : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.erfinv.default](args = (%uniform,), kwargs = {})
#   %mul : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%erfinv, 0.007071067811865476), kwargs = {})
#   %add : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mul, 0.0), kwargs = {})
#   %convert_element_type : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add, torch.float32), kwargs = {})
#   %clamp_min : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.clamp_min.default](args = (%convert_element_type, -0.015), kwargs = {})
#   %clamp_max : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.clamp_max.default](args = (%clamp_min, 0.015), kwargs = {})
#   %convert_element_type_1 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%clamp_max, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(3):
            ...
            triton_poi_fused_randn_0.run(buf0, buf2, 0, 50331648, stream=stream3)
            del buf0
            ...
            # Topologically Sorted Source Nodes: [erfinv_, mul_, add_, clamp_], Original ATen: [aten.erfinv, aten.mul, aten.add, aten.clamp]
            stream3 = get_raw_stream(3)
            triton_poi_fused_add_clamp_erfinv_mul_1.run(buf5, 50331648, stream=stream3)
        return (buf5, )
