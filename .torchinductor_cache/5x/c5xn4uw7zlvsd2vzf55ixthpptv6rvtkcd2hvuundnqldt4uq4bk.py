...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/s2/cs2k6pfv6vy2wgx5cq47at6ebpwc344wzx6vbuftajeuzy32jlwb.py
...
#   %sum_1 : Tensor "f32[350, 1000, 112][112000, 112, 1]cuda:3" = PlaceHolder[target=sum_1]
...
#   %remainder : Tensor "i64[350, 999, 8][7992, 8, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 8191), kwargs = {})
...
#   %embedding_1 : Tensor "bf16[350, 1000, 8, 112][896000, 896, 112, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%arg6_1, %add_1), kwargs = {})
#   %sum_1 : Tensor "f32[350, 1000, 112][112000, 112, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%embedding_1, [-2]), kwargs = {dtype: torch.float32})
#   %mul_3 : Tensor "f32[350, 1000, 112][112000, 112, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[350, 1000, 112][112000, 112, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_3, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            # Topologically Sorted Source Nodes: [setitem, setitem_1, remainder, add, arange, mul_2, add_1, embedding_1, sum_1, mul_3, linear], Original ATen: [aten.select, aten.lift_fresh, aten.fill, aten.slice, aten.remainder, aten.add, aten.copy, aten.arange, aten.mul, aten.embedding, aten.sum, aten._to_copy]
            stream3 = get_raw_stream(3)
            triton_per_fused__to_copy_add_arange_copy_embedding_fill_lift_fresh_mul_remainder_select_slice_sum_1.run(buf1, arg6_1, buf3, 39200000, 8, stream=stream3)
            del arg6_1
            del buf1
