...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/fu/cfuzanfhxyxh7hx2mx4y4v6um4vl62uvxz6yvusiix46rjvtnsv7.py
...
# Graph fragment:
#   %full_default_5 : Tensor "f32[65536, 120][120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([65536, 120], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:6, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/vh/cvh7imlraidkz7akvwd7oaeyt6ff7evcpeivzeznnc473aoukzly.py
...
#   where => where
# Graph fragment:
#   %add_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:6" = PlaceHolder[target=add_1]
#   %mm_13 : Tensor "bf16[131072, 120][120, 1]cuda:6" = PlaceHolder[target=mm_13]
#   %index_put : Tensor "f32[65536, 120][120, 1]cuda:6" = PlaceHolder[target=index_put]
#   %view_30 : Tensor "bf16[128, 1024, 120][122880, 120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm_13, [128, 1024, 120]), kwargs = {})
#   %convert_element_type_75 : Tensor "f32[128, 1024, 120][122880, 120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_30, torch.float32), kwargs = {})
#   %mul_45 : Tensor "f32[128, 1024, 120][122880, 120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.3535533905932738), kwargs = {})
#   %unsqueeze_6 : Tensor "f32[128, 1024, 1, 120][122880, 120, 120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%mul_45, -2), kwargs = {})
#   %expand : Tensor "f32[128, 1024, 8, 120][122880, 120, 0, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_6, [128, 1024, 8, 120]), kwargs = {})
#   %convert_element_type_default : Tensor "f32[128, 1024, 8, 120][983040, 960, 120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%expand, torch.float32), kwargs = {})
#   %eq : Tensor "b8[128, 1024, 8][8192, 8, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%add_1, -1), kwargs = {})
#   %unsqueeze_7 : Tensor "b8[128, 1024, 8, 1][8192, 8, 1, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
#   %full_default_4 : Tensor "f32[][]cuda:6"[num_users=2] = call_function[target=torch.ops.aten.full.default](args = ([], 0.0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:6, pin_memory: False})
#   %where : Tensor "f32[128, 1024, 8, 120][983040, 960, 120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_7, %full_default_4, %convert_element_type_default), kwargs = {})
#   %full_default_5 : Tensor "f32[65536, 120][120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([65536, 120], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:6, pin_memory: False})
#   %index_put : Tensor "f32[65536, 120][120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_5, [%add_1], %where, True), kwargs = {})
#   return %buf43
...
@triton_heuristics.pointwise(
    ...
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i64', 'in_ptr1': '*bf16', 'out_ptr0': '*fp32', 'ynumel': 'i32', 'xnumel': 'i32', 'YBLOCK': 'constexpr', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=6, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]]}]},
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/4p/c4pmdlemv3lp7gnom45ccfvlnxbu32cjxdnytxgx3lg3kjpqazfc.py
...
# Graph fragment:
#   %buf43 : Tensor "f32[65536, 120][120, 1]cuda:6" = PlaceHolder[target=buf43]
#   %convert_element_type_80 : Tensor "bf16[65536, 120][120, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(6):
            ...
            # Topologically Sorted Source Nodes: [full_default_5], Original ATen: [aten.embedding_dense_backward]
            stream6 = get_raw_stream(6)
            triton_poi_fused_embedding_dense_backward_13.run(buf42, 7864320, stream=stream6)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_45, unsqueeze_6, expand, convert_element_type_default, eq, unsqueeze_7, full_default_4, where, full_default_5, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream6 = get_raw_stream(6)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_14.run(add_1, buf41, buf42, 1048576, 120, stream=stream6)
            del add_1
            del buf41
            buf44 = empty_strided_cuda((65536, 120), (120, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream6 = get_raw_stream(6)
            triton_poi_fused_embedding_dense_backward_15.run(buf42, buf44, 7864320, stream=stream6)
            del buf42
...
def get_args():
    ...
    add_1 = rand_strided((128, 1024, 8), (8192, 8, 1), device='cuda:6', dtype=torch.int64)
    view = rand_strided((131072, 120), (120, 1), device='cuda:6', dtype=torch.bfloat16)
    ...
    permute_34 = rand_strided((384, 120), (120, 1), device='cuda:6', dtype=torch.bfloat16)
