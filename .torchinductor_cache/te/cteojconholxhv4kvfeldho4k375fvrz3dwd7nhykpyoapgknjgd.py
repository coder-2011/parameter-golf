...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/eg/cegqd7vspvgzbuh7pgov23h4vepumcrsyyb72ttpzdme3svdkwdu.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/kb/ckbyxlvazkqlquxtxwklypk5mfheznqcj5d2hyr7i3oud4qgcpzg.py
...
#   %remainder : Tensor "i64[128, 1023, 8][8184, 8, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
...
#   %add_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/xl/cxli4jqgwyu6pgelw7noctqyzouk4eldxs37kt6fj6wyd5akd2sy.py
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(7):
            ...
            triton_poi_fused_bitwise_xor_lift_fresh_mul_slice_unsqueeze_0.run(primals_5, buf2, 130944, 8, stream=stream7)
            buf3 = empty_strided_cuda((128, 1024, 8), (8192, 8, 1), torch.int64)
            # Topologically Sorted Source Nodes: [setitem, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.lift_fresh, aten.select, aten.fill, aten.remainder, aten.add, aten.slice, aten.copy, aten.arange, aten.mul]
            stream7 = get_raw_stream(7)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_1.run(buf2, buf3, 1048576, stream=stream7)
            del buf2
            buf0 = empty_strided_cuda((128, 1024, 384), (393216, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream7 = get_raw_stream(7)
            triton_poi_fused_embedding_2.run(primals_5, primals_6, buf0, 131072, 384, stream=stream7)
            del primals_6
