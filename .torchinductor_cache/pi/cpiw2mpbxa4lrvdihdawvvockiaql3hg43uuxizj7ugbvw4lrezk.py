...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/e5/ce5jnpmuxpfqexqx4a2v7m3adlalie4ekjwlofilywth2t6aroyy.py
# Topologically Sorted Source Nodes: [mul, linear], Original ATen: [aten.mul, aten.t]
# Source node to ATen node mapping:
#   linear => permute
#   mul => mul
# Graph fragment:
#   %primals_1 : Tensor "bf16[8192, 384][384, 1]cuda:3" = PlaceHolder[target=primals_1]
#   %mul : Tensor "bf16[8192, 384][384, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_1, 1.0), kwargs = {})
#   %permute : Tensor "bf16[384, 8192][1, 384]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%mul, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ze/czectaupxj3yiyjby3m4kjbuibmjp7zqzphopkhw56qfwoh6rank.py
...
#   %sum_1 : Tensor "f32[131072, 1][1, 131072]cuda:3" = PlaceHolder[target=sum_1]
...
#   %tanh : Tensor "f32[131072, 8192][8192, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.tanh.default](args = (%div,), kwargs = {})
#   %mul_tensor : Tensor "f32[131072, 8192][8192, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%tanh, 1), kwargs = {})
#   %amax_default : Tensor "f32[131072, 1][1, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.amax.default](args = (%mul_tensor, [1], True), kwargs = {})
...
#   %mul_tensor_1 : Tensor "f32[131072, 8192][8192, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sub_tensor, 30.0), kwargs = {})
#   %exp : Tensor "f32[131072, 8192][8192, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.exp.default](args = (%mul_tensor_1,), kwargs = {})
#   %sum_1 : Tensor "f32[131072, 1][1, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%exp, [1], True), kwargs = {})
#   %log : Tensor "f32[131072, 1][1, 1]cuda:3"[num_users=2] = call_function[target=torch.ops.aten.log.default](args = (%sum_1,), kwargs = {})
...
@triton_heuristics.reduction(
    ...
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*bf16', 'out_ptr0': '*fp32', 'xnumel': 'i32', 'r0_numel': 'i32', 'XBLOCK': 'constexpr', 'R0_BLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=3, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]], (4,): [['tt.divisibility', 16]]}]},
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/22/c22g263uzqvdxhjafdu4ypc6ebdd3kfj3fhvub3x5u3elb7pjgz3.py
...
#   %ne : Tensor "b8[131072][1]cuda:3"[num_users=3] = call_function[target=torch.ops.aten.ne.Scalar](args = (%primals_3, -100), kwargs = {})
...
#   %where : Tensor "i64[131072][1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%ne, %primals_3, %full_default), kwargs = {})
#   %unsqueeze : Tensor "i64[131072, 1][1, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%where, 1), kwargs = {})
#   %gather : Tensor "f32[131072, 1][1, 1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.gather.default](args = (%sub_1, 1, %unsqueeze), kwargs = {})
#   %squeeze : Tensor "f32[131072][1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.squeeze.dim](args = (%gather, 1), kwargs = {})
#   %neg : Tensor "f32[131072][1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.neg.default](args = (%squeeze,), kwargs = {})
...
#   %where_1 : Tensor "f32[131072][1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%ne, %neg, %full_default_1), kwargs = {})
#   %sum_2 : Tensor "i64[][]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sum.default](args = (%ne,), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/2d/c2dqgmo6q35e5u3lhuaieeysm6dgjm75zhel46zgyhoh5pbvv2as.py
# Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.sub, aten._log_softmax, aten.nll_loss_forward]
# Source node to ATen node mapping:
...
#   truediv => div
# Graph fragment:
#   %buf10 : Tensor "f32[16][1]cuda:3" = PlaceHolder[target=buf10]
#   %buf6 : Tensor "i64[16][1]cuda:3" = PlaceHolder[target=buf6]
...
#   %where_1 : Tensor "f32[131072][1]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%ne, %neg, %full_default_1), kwargs = {})
#   %sum_2 : Tensor "i64[][]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sum.default](args = (%ne,), kwargs = {})
#   %convert_element_type_3 : Tensor "f32[][]cuda:3"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%sum_2, torch.float32), kwargs = {})
#   %sum_3 : Tensor "f32[][]cuda:3"[num_users=1] = call_function[target=torch.ops.aten.sum.default](args = (%where_1,), kwargs = {})
...
@triton_heuristics.persistent_reduction(
    ...
    reduction_hint=ReductionHint.INNER,
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*fp32', 'in_ptr1': '*i64', 'out_ptr1': '*fp32', 'xnumel': 'constexpr', 'r0_numel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=3, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {'xnumel': 1}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]], (5,): [['tt.divisibility', 16]]}]},
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(3):
            ...
            triton_poi_fused_mul_t_0.run(primals_1, buf0, 3145728, stream=stream3)
            del primals_1
            ...
            # Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.amax, aten.sub, aten._log_softmax]
            stream3 = get_raw_stream(3)
            triton_red_fused__log_softmax__to_copy_amax_div_mul_sub_tanh_1.run(buf4, buf1, buf2, 131072, 8192, stream=stream3)
            buf6 = empty_strided_cuda((16, ), (1, ), torch.int64)
            buf10 = empty_strided_cuda((16, ), (1, ), torch.float32)
            # Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.sub, aten._log_softmax, aten.nll_loss_forward]
            stream3 = get_raw_stream(3)
            triton_red_fused__log_softmax__to_copy_div_mul_nll_loss_forward_sub_tanh_2.run(primals_3, buf1, buf2, buf4, buf6, buf10, 16, 8192, stream=stream3)
            buf11 = empty_strided_cuda((), (), torch.float32)
            buf8 = empty_strided_cuda((), (), torch.float32)
            buf12 = buf11; del buf11  # reuse
            # Topologically Sorted Source Nodes: [float_1, truediv, tanh, cross_entropy], Original ATen: [aten._to_copy, aten.div, aten.tanh, aten.mul, aten.sub, aten._log_softmax, aten.nll_loss_forward]
            stream3 = get_raw_stream(3)
            triton_per_fused__log_softmax__to_copy_div_mul_nll_loss_forward_sub_tanh_3.run(buf12, buf10, buf6, buf8, 1, 16, stream=stream3)
            del buf10
            del buf6
...
def get_args():
    ...
    primals_1 = rand_strided((8192, 384), (384, 1), device='cuda:3', dtype=torch.bfloat16)
