...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/dn/cdnawphat4hjk4g5dr7nymq7yttdazt352tahzdw6mqqodbdgbyf.py
...
#   %lift_fresh_copy : Tensor "i64[7][1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.lift_fresh_copy.default](args = (%_tensor_constant0,), kwargs = {})
#   %lift_fresh_copy_1 : Tensor "i64[7][1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.lift_fresh_copy.default](args = (%_tensor_constant1,), kwargs = {})
...
#   %mul : Tensor "i64[128, 1023, 7][7161, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %lift_fresh_copy), kwargs = {})
...
#   %mul_1 : Tensor "i64[128, 1023, 7][7161, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze_1, %lift_fresh_copy_1), kwargs = {})
#   %bitwise_xor : Tensor "i64[128, 1023, 7][7161, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.bitwise_xor.Tensor](args = (%mul, %mul_1), kwargs = {})
#   return %bitwise_xor
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/3v/c3vpfiortlis4h3fv57jxjzuha2icgahoopgu223nrs6nfpoykly.py
...
# Graph fragment:
#   %bitwise_xor : Tensor "i64[128, 1023, 7][7168, 7, 1]cuda:6" = PlaceHolder[target=bitwise_xor]
...
#   %select : Tensor "i64[128, 7][7168, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%empty, 1, 0), kwargs = {})
#   %copy : Tensor "i64[128, 7][7168, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%select, %full_default), kwargs = {})
#   %select_scatter_default : Tensor "i64[128, 1024, 7][7168, 7, 1]cuda:6"[num_users=2] = call_function[target=torch.ops.aten.select_scatter.default](args = (%empty, %copy, 1, 0), kwargs = {})
#   %remainder : Tensor "i64[128, 1023, 7][7161, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 8191), kwargs = {})
#   %add : Tensor "i64[128, 1023, 7][7161, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%remainder, 1), kwargs = {})
#   %slice_4 : Tensor "i64[128, 1023, 7][7168, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%select_scatter_default, 1, 1, 9223372036854775807), kwargs = {})
#   %copy_1 : Tensor "i64[128, 1023, 7][7168, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%slice_4, %add), kwargs = {})
#   %slice_scatter_default : Tensor "i64[128, 1024, 7][7168, 7, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.slice_scatter.default](args = (%select_scatter_default, %copy_1, 1, 1, 9223372036854775807), kwargs = {})
#   %iota : Tensor "i64[7][1]cuda:6"[num_users=1] = call_function[target=torch.ops.prims.iota.default](args = (7,), kwargs = {start: 0, step: 1, dtype: torch.int64, device: cuda:6, requires_grad: False})
#   %mul_2 : Tensor "i64[7][1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%iota, 8192), kwargs = {})
#   %add_1 : Tensor "i64[128, 1024, 7][7168, 7, 1]cuda:6"[num_users=2] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/e7/ce732aujdobplu2lwikpfkxxeqy7vbsc2utcvtkjgfnsintbriiz.py
...
#   %primals_7 : Tensor "bf16[57344, 128][128, 1]cuda:6" = PlaceHolder[target=primals_7]
#   %sum_1 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:6" = PlaceHolder[target=sum_1]
#   %embedding_1 : Tensor "bf16[128, 1024, 7, 128][917504, 896, 128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_7, %add_1), kwargs = {})
...
#   %mul_3 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:6"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.37796447300922725), kwargs = {})
...
@triton_heuristics.pointwise(
    ...
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*i64', 'in_ptr1': '*bf16', 'out_ptr1': '*bf16', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=6, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]]}]},
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(6):
            ...
            # Topologically Sorted Source Nodes: [getitem_4, getitem_5, tensor, tensor_1, getitem_6, mul, getitem_7, mul_1, bitwise_xor], Original ATen: [aten.slice, aten.lift_fresh, aten.unsqueeze, aten.mul, aten.bitwise_xor]
            stream6 = get_raw_stream(6)
            triton_poi_fused_bitwise_xor_lift_fresh_mul_slice_unsqueeze_1.run(primals_5, buf2, 916608, stream=stream6)
            buf3 = empty_strided_cuda((128, 1024, 7), (7168, 7, 1), torch.int64)
            # Topologically Sorted Source Nodes: [setitem, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.lift_fresh, aten.select, aten.fill, aten.remainder, aten.add, aten.slice, aten.copy, aten.arange, aten.mul]
            stream6 = get_raw_stream(6)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_2.run(buf2, buf3, 917504, stream=stream6)
            del buf2
            buf6 = empty_strided_cuda((128, 1024, 128), (131072, 128, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding_1, sum_1, mul_3, linear], Original ATen: [aten.embedding, aten.sum, aten.mul, aten._to_copy]
            stream6 = get_raw_stream(6)
            triton_poi_fused__to_copy_embedding_mul_sum_3.run(buf3, primals_7, buf6, 16777216, stream=stream6)
            del primals_7
...
def get_args():
    ...
    primals_7 = rand_strided((57344, 128), (128, 1), device='cuda:6', dtype=torch.bfloat16)
