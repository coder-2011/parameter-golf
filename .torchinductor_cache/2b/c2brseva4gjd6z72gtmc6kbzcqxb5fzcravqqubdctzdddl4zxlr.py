...
# Graph fragment:
#   %full_default_5 : Tensor "f32[65536, 112][112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([65536, 112], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/sb/csbbxuikulatohlq642ofxdupcjsbtwoyrwhohmaax3mdje6cefq.py
...
#   where => where
# Graph fragment:
#   %add_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:4" = PlaceHolder[target=add_1]
#   %mm_13 : Tensor "bf16[131072, 112][112, 1]cuda:4" = PlaceHolder[target=mm_13]
#   %index_put : Tensor "f32[65536, 112][112, 1]cuda:4" = PlaceHolder[target=index_put]
#   %view_30 : Tensor "bf16[128, 1024, 112][114688, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm_13, [128, 1024, 112]), kwargs = {})
#   %convert_element_type_75 : Tensor "f32[128, 1024, 112][114688, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_30, torch.float32), kwargs = {})
#   %mul_45 : Tensor "f32[128, 1024, 112][114688, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.3535533905932738), kwargs = {})
#   %unsqueeze_6 : Tensor "f32[128, 1024, 1, 112][114688, 112, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%mul_45, -2), kwargs = {})
#   %expand : Tensor "f32[128, 1024, 8, 112][114688, 112, 0, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_6, [128, 1024, 8, 112]), kwargs = {})
#   %convert_element_type_default : Tensor "f32[128, 1024, 8, 112][917504, 896, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%expand, torch.float32), kwargs = {})
#   %eq : Tensor "b8[128, 1024, 8][8192, 8, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%add_1, -1), kwargs = {})
#   %unsqueeze_7 : Tensor "b8[128, 1024, 8, 1][8192, 8, 1, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
#   %full_default_4 : Tensor "f32[][]cuda:4"[num_users=2] = call_function[target=torch.ops.aten.full.default](args = ([], 0.0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   %where : Tensor "f32[128, 1024, 8, 112][917504, 896, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_7, %full_default_4, %convert_element_type_default), kwargs = {})
#   %full_default_5 : Tensor "f32[65536, 112][112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([65536, 112], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:4, pin_memory: False})
#   %index_put : Tensor "f32[65536, 112][112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_5, [%add_1], %where, True), kwargs = {})
#   return %buf43
...
# Graph fragment:
#   %buf43 : Tensor "f32[65536, 112][112, 1]cuda:4" = PlaceHolder[target=buf43]
#   %convert_element_type_80 : Tensor "bf16[65536, 112][112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_45, unsqueeze_6, expand, convert_element_type_default, eq, unsqueeze_7, full_default_4, where, full_default_5, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream4 = get_raw_stream(4)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_14.run(add_1, buf41, buf42, 1048576, 112, stream=stream4)
            del add_1
            del buf41
...
def get_args():
    ...
    add_1 = rand_strided((128, 1024, 8), (8192, 8, 1), device='cuda:4', dtype=torch.int64)
    view = rand_strided((131072, 112), (112, 1), device='cuda:4', dtype=torch.bfloat16)
    ...
    permute_34 = rand_strided((384, 112), (112, 1), device='cuda:4', dtype=torch.bfloat16)
