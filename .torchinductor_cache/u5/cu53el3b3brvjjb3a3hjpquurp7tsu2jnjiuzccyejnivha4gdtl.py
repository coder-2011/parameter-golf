...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/wz/cwzhwq2jxabznaafzuxnc2pyy7pgry7gueyx7xq3g67n4z6xr6dm.py
...
# Graph fragment:
#   %primals_1 : Tensor "bf16[1024, 384][384, 1]cuda:2" = PlaceHolder[target=primals_1]
#   %convert_element_type_default_5 : Tensor "bf16[1024, 384][384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_1, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[384, 1024][1, 384]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_5, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/db/cdbl5wnscoxsh5dgrnzaqmxhksd2b6dda2kj5ii5zhwplv7no5rb.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/yn/cynvhbe4crmvsbqxnmfvwbpqlz5yg33h5hh3bf44y6xch53gim6f.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/lc/clcujldo7fgai3wylihprfx6zaiumf4hgcxxckeryfim54vz2rk6.py
...
#   mul_22 => mul_2
# Graph fragment:
#   %primals_6 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2" = PlaceHolder[target=primals_6]
...
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_4, %primals_5), kwargs = {})
#   %add : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_6, %mul_1), kwargs = {})
#   %mul_2 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_7, %view_3), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused__to_copy_t_0.run(primals_1, buf0, 393216, stream=stream2)
            del primals_1
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_0.run(primals_3, buf3, 393216, stream=stream2)
            del primals_3
            buf4 = empty_strided_cuda((128, 1024, 1024), (1048576, 1024, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear, leaky_relu, square, linear_1], Original ATen: [aten._unsafe_view, aten.leaky_relu, aten.pow, aten._to_copy]
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy__unsafe_view_leaky_relu_pow_2.run(buf2, buf4, 134217728, stream=stream2)
            ...
            # Topologically Sorted Source Nodes: [linear_1, mul, add, mul_22, add_1], Original ATen: [aten._unsafe_view, aten.mul, aten.add]
            stream2 = get_raw_stream(2)
            triton_poi_fused__unsafe_view_add_mul_3.run(primals_6, primals_4, primals_5, primals_7, buf5, buf6, 50331648, stream=stream2)
            del primals_6
...
def get_args():
    ...
    primals_1 = rand_strided((1024, 384), (384, 1), device='cuda:2', dtype=torch.bfloat16)
    primals_2 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:2', dtype=torch.float32)
    primals_3 = rand_strided((384, 1024), (1024, 1), device='cuda:2', dtype=torch.bfloat16)
    ...
    primals_6 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:2', dtype=torch.float32)
