...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/oa/coagerkl4tonms3gdoabscm4ugh3w5w2afxl5iyhftllsm4cpx2c.py
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/fz/cfzbljcdfbzeofv2bbotvwq3sfhexfukki6i3zejvq2utl5v455b.py
...
#   %remainder : Tensor "i64[128, 1023, 8][8184, 8, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 16383), kwargs = {})
...
#   %add_1 : Tensor "i64[128, 1024, 8][8192, 8, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/mg/cmgt5pkaxt2d643lw2fukxdpcfrbcrhuuzzwurgsxtmju2us647w.py
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(5):
            ...
            triton_poi_fused_bitwise_xor_lift_fresh_mul_slice_unsqueeze_0.run(primals_5, buf2, 130944, 8, stream=stream5)
            buf3 = empty_strided_cuda((128, 1024, 8), (8192, 8, 1), torch.int64)
            # Topologically Sorted Source Nodes: [setitem, remainder, add, setitem_1, arange, mul_2, add_1], Original ATen: [aten.lift_fresh, aten.select, aten.fill, aten.remainder, aten.add, aten.slice, aten.copy, aten.arange, aten.mul]
            stream5 = get_raw_stream(5)
            triton_poi_fused_add_arange_copy_fill_lift_fresh_mul_remainder_select_slice_1.run(buf2, buf3, 1048576, stream=stream5)
            del buf2
            buf0 = empty_strided_cuda((128, 1024, 384), (393216, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream5 = get_raw_stream(5)
            triton_poi_fused_embedding_2.run(primals_5, primals_6, buf0, 131072, 384, stream=stream5)
            del primals_6
