...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/se/csefp6scnefoka37ip4xhyc2lwgh6nhadnfff2dtvsqzyzvbblay.py
...
# Graph fragment:
#   %inductor_seeds_default : Tensor "i64[1][1]cuda:0" = PlaceHolder[target=inductor_seeds_default]
#   %inductor_random_default : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0" = PlaceHolder[target=inductor_random_default]
...
#   %inductor_random_default : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.inductor_random.default](args = ([128, 1024, 384], %inductor_lookup_seed_default, randn), kwargs = {})
#   %convert_element_type_default : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%inductor_random_default, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/as/casb4rz43n72bso4vqypptth6fbdnrrcdvym2xjdhj2bjr5bmekc.py
...
#   mul_ => mul
# Graph fragment:
#   %uniform : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0" = PlaceHolder[target=uniform]
#   %erfinv : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.erfinv.default](args = (%uniform,), kwargs = {})
#   %mul : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%erfinv, 0.007071067811865476), kwargs = {})
#   %add : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mul, 0.0), kwargs = {})
#   %convert_element_type : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add, torch.float32), kwargs = {})
#   %clamp_min : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.clamp_min.default](args = (%convert_element_type, -0.015), kwargs = {})
#   %clamp_max : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.aten.clamp_max.default](args = (%clamp_min, 0.015), kwargs = {})
#   %convert_element_type_1 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:0"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%clamp_max, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(0):
            ...
            triton_poi_fused_randn_0.run(buf0, buf2, 0, 50331648, stream=stream0)
            del buf0
            ...
            # Topologically Sorted Source Nodes: [erfinv_, mul_, add_, clamp_], Original ATen: [aten.erfinv, aten.mul, aten.add, aten.clamp]
            stream0 = get_raw_stream(0)
            triton_poi_fused_add_clamp_erfinv_mul_1.run(buf5, 50331648, stream=stream0)
        return (buf5, )
