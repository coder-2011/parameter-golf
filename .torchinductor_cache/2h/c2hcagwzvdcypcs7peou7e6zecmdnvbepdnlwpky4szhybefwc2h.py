...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/67/c672arwz65usfmmu5hktx54rzklx2dfamtxd2z4u7srnz3ioybxy.py
...
#   %mul : Tensor "i64[512, 999, 7][6993, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %lift_fresh_copy), kwargs = {})
...
#   %mul_1 : Tensor "i64[512, 999, 7][6993, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze_1, %lift_fresh_copy_1), kwargs = {})
#   %bitwise_xor : Tensor "i64[512, 999, 7][6993, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.bitwise_xor.Tensor](args = (%mul, %mul_1), kwargs = {})
#   return %bitwise_xor
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/cg/ccg37zlo4w3n5vtplihrq6ynlj3tmgiyv3r2rjtbnrfmb24t2uzo.py
...
#   sum_1 => sum_1
# Graph fragment:
#   %bitwise_xor : Tensor "i64[512, 999, 7][7008, 7, 1]cuda:1" = PlaceHolder[target=bitwise_xor]
#   %arg6_1 : Tensor "bf16[57344, 128][128, 1]cuda:1" = PlaceHolder[target=arg6_1]
#   %sum_1 : Tensor "f32[512, 1000, 128][128000, 128, 1]cuda:1" = PlaceHolder[target=sum_1]
#   %select : Tensor "i64[512, 7][7000, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%empty, 1, 0), kwargs = {})
...
#   %copy : Tensor "i64[512, 7][7000, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%select, %full_default), kwargs = {})
#   %select_scatter_default : Tensor "i64[512, 1000, 7][7000, 7, 1]cuda:1"[num_users=2] = call_function[target=torch.ops.aten.select_scatter.default](args = (%empty, %copy, 1, 0), kwargs = {})
#   %slice_8 : Tensor "i64[512, 999, 7][7000, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%select_scatter_default, 1, 1, 9223372036854775807), kwargs = {})
#   %remainder : Tensor "i64[512, 999, 7][6993, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 8191), kwargs = {})
#   %add : Tensor "i64[512, 999, 7][6993, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%remainder, 1), kwargs = {})
#   %copy_1 : Tensor "i64[512, 999, 7][7000, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%slice_8, %add), kwargs = {})
#   %slice_scatter_default : Tensor "i64[512, 1000, 7][7000, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.slice_scatter.default](args = (%select_scatter_default, %copy_1, 1, 1, 9223372036854775807), kwargs = {})
...
#   %add_1 : Tensor "i64[512, 1000, 7][7000, 7, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   %embedding_1 : Tensor "bf16[512, 1000, 7, 128][896000, 896, 128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%arg6_1, %add_1), kwargs = {})
...
#   %mul_3 : Tensor "f32[512, 1000, 128][128000, 128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.37796447300922725), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            triton_poi_fused_bitwise_xor_lift_fresh_mul_slice_unsqueeze_0.run(arg4_1, buf1, 3580416, stream=stream1)
            buf3 = empty_strided_cuda((512, 1000, 128), (128000, 128, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [setitem, setitem_1, remainder, add, arange, mul_2, add_1, embedding_1, sum_1, mul_3, linear], Original ATen: [aten.select, aten.lift_fresh, aten.fill, aten.slice, aten.remainder, aten.add, aten.copy, aten.arange, aten.mul, aten.embedding, aten.sum, aten._to_copy]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_add_arange_copy_embedding_fill_lift_fresh_mul_remainder_select_slice_sum_1.run(buf1, arg6_1, buf3, 65536000, stream=stream1)
            del arg6_1
            del buf1
...
def get_args():
    ...
    arg6_1 = rand_strided((57344, 128), (128, 1), device='cuda:1', dtype=torch.bfloat16)
