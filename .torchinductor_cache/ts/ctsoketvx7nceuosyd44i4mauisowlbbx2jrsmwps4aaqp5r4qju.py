...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/uz/cuzj3nsgaih3f2ciggxw4rhnzpgqhae5oxs2hoff6yqxfkdtj5w2.py
...
#   %view_3 : Tensor "bf16[131072, 384][384, 1]cuda:7"[num_users=2] = call_function[target=torch.ops.aten.reshape.default](args = (%tangents_1, [-1, 384]), kwargs = {})
#   %permute_1 : Tensor "bf16[1536, 384][1, 1536]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%primals_1, [1, 0]), kwargs = {})
#   %clone : Tensor "bf16[1536, 384][384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.clone.default](args = (%permute_1,), kwargs = {memory_format: torch.contiguous_format})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(7):
            ...
            triton_poi_fused_clone_permute_view_0.run(primals_1, buf2, 1536, 384, stream=stream7)
            del primals_1
            # Topologically Sorted Source Nodes: [reshape, getattr_2, contiguous_2, triton_kernel_wrapper_mutation], Original ATen: [aten.view, aten.permute, aten.clone]
            stream7 = get_raw_stream(7)
            _leaky_relu_sq_matmul_kernel_0.run(reinterpret_tensor(tangents_1, (131072, 384), (384, 1), 0), buf2, buf1, getitem, 24576, 1, 1, stream=stream7)
            del getitem
...
def get_args():
    ...
    primals_3 = rand_strided((1536, 384), (384, 1), device='cuda:7', dtype=torch.bfloat16)
    view = rand_strided((131072, 384), (384, 1), device='cuda:7', dtype=torch.bfloat16)
    getitem = rand_strided((131072, 1536), (1536, 1), device='cuda:7', dtype=torch.bfloat16)
    getitem_1 = rand_strided((131072, 1536), (1536, 1), device='cuda:7', dtype=torch.bfloat16)
