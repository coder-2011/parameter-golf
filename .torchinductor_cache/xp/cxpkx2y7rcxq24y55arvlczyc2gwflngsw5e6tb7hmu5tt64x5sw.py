...
_tensor_constant0 = None  # device(type='cuda', index=2) torch.int64 (12,) (1,) 7b425655f750
_tensor_constant1 = None  # device(type='cuda', index=2) torch.int64 (12,) (1,) 7b4256577700


# kernel path: /workspace/parameter-golf/.torchinductor_cache/74/c74wzgiu3hyr6fplzn3qzyfigqs655xejtldkxjtqqus2ljugnyi.py
...
#   %select : Tensor "i64[349, 12][12000, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.select.int](args = (%empty, 1, 0), kwargs = {})
...
#   %copy : Tensor "i64[349, 12][12000, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%select, %full_default), kwargs = {})
#   %select_scatter_default : Tensor "i64[349, 1000, 12][12000, 12, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.select_scatter.default](args = (%empty, %copy, 1, 0), kwargs = {})
#   %slice_8 : Tensor "i64[349, 999, 12][12000, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%select_scatter_default, 1, 1, 9223372036854775807), kwargs = {})
...
#   %mul : Tensor "i64[349, 999, 12][11988, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze, %lift_fresh_copy), kwargs = {})
...
#   %mul_1 : Tensor "i64[349, 999, 12][11988, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%unsqueeze_1, %lift_fresh_copy_1), kwargs = {})
#   %bitwise_xor : Tensor "i64[349, 999, 12][11988, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.bitwise_xor.Tensor](args = (%mul, %mul_1), kwargs = {})
#   %remainder : Tensor "i64[349, 999, 12][11988, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.remainder.Scalar](args = (%bitwise_xor, 32767), kwargs = {})
#   %add : Tensor "i64[349, 999, 12][11988, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%remainder, 1), kwargs = {})
#   %copy_1 : Tensor "i64[349, 999, 12][12000, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.copy.default](args = (%slice_8, %add), kwargs = {})
#   %slice_scatter_default : Tensor "i64[349, 1000, 12][12000, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.slice_scatter.default](args = (%select_scatter_default, %copy_1, 1, 1, 9223372036854775807), kwargs = {})
...
#   %add_1 : Tensor "i64[349, 1000, 12][12000, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%slice_scatter_default, %mul_2), kwargs = {})
#   return %add_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ru/crubprtirzqbda6onoi2mgbyavfpp33jd2xsba7wqzdzbvdh3ey4.py
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused_add_arange_bitwise_xor_copy_fill_lift_fresh_mul_remainder_select_slice_unsqueeze_0.run(arg4_1, _tensor_constant0, _tensor_constant1, buf1, 349000, 12, stream=stream2)
            buf2 = empty_strided_cuda((349, 1000, 384), (384000, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [embedding], Original ATen: [aten.embedding]
            stream2 = get_raw_stream(2)
            triton_poi_fused_embedding_1.run(arg4_1, arg5_1, buf2, 134016000, stream=stream2)
            del arg4_1
            del arg5_1
