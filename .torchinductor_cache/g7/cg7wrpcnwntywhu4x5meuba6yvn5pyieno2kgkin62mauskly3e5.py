...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ek/cekoqdzqky5feudaqye3byxu2rd44rhfqe3dozygtq3mnpu4vwuv.py
...
# Graph fragment:
#   %primals_1 : Tensor "bf16[384, 1536][1536, 1]cuda:5" = PlaceHolder[target=primals_1]
#   %convert_element_type_default_1 : Tensor "bf16[384, 1536][1536, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_1, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[1536, 384][1, 1536]cuda:5"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_1, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ak/cakohzdulaip4t2scjyj7appypszsrbot5fd65igdpxgz2osndgy.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
# Source node to ATen node mapping:
#   linear => convert_element_type_2
# Graph fragment:
#   %primals_2 : Tensor "f32[128, 1024, 1536][1572864, 1536, 1]cuda:5" = PlaceHolder[target=primals_2]
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 1536][1572864, 1536, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_2, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/cu/ccut23dsj3decexcxg5osagb3oodo3yjcvh2etcrfqllnbq6vd4f.py
...
#   mul_1 => mul_1
# Graph fragment:
#   %primals_5 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5" = PlaceHolder[target=primals_5]
...
#   %mul : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_3, %primals_4), kwargs = {})
#   %add : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%primals_5, %mul), kwargs = {})
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_6, %view_1), kwargs = {})
#   %add_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%add, %mul_1), kwargs = {})
#   return %add_1
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(5):
            ...
            triton_poi_fused__to_copy_t_0.run(primals_1, buf0, 589824, stream=stream5)
            del primals_1
            buf1 = empty_strided_cuda((128, 1024, 1536), (1572864, 1536, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
            stream5 = get_raw_stream(5)
            triton_poi_fused__to_copy_1.run(primals_2, buf1, 201326592, stream=stream5)
            del primals_2
            ...
            # Topologically Sorted Source Nodes: [linear, mul, add, mul_1, add_1], Original ATen: [aten._unsafe_view, aten.mul, aten.add]
            stream5 = get_raw_stream(5)
            triton_poi_fused__unsafe_view_add_mul_2.run(primals_5, primals_3, primals_4, primals_6, buf2, buf3, 50331648, stream=stream5)
            del primals_5
...
def get_args():
    ...
    primals_1 = rand_strided((384, 1536), (1536, 1), device='cuda:5', dtype=torch.bfloat16)
    primals_2 = rand_strided((128, 1024, 1536), (1572864, 1536, 1), device='cuda:5', dtype=torch.float32)
    ...
    primals_5 = rand_strided((128, 1024, 384), (393216, 384, 1), device='cuda:5', dtype=torch.float32)
