...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/2k/c2kpvyc4zkaactfr2daqvupnpi2dabtz4sybk7acmciisqxdkxgq.py
...
# Graph fragment:
#   %full_default_4 : Tensor "f32[196608, 128][128, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([196608, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:2, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ko/ckoqscpb7dzqvliexq4sfxjwqp4vy7qphqjjthns2nfqnkdpbblg.py
...
#   %index_put : Tensor "f32[196608, 128][128, 1]cuda:2" = PlaceHolder[target=index_put]
...
#   %full_default_4 : Tensor "f32[196608, 128][128, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([196608, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:2, pin_memory: False})
#   %index_put : Tensor "f32[196608, 128][128, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_4, [%primals_1], %where, True), kwargs = {})
#   return %buf44
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/32/c32bt3mluuwmsezh5zpj7twfdcmc3h2lx3dbusomx7qo2qir5tdd.py
...
# Graph fragment:
#   %buf44 : Tensor "f32[196608, 128][128, 1]cuda:2" = PlaceHolder[target=buf44]
#   %convert_element_type_80 : Tensor "bf16[196608, 128][128, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            # Topologically Sorted Source Nodes: [full_default_4], Original ATen: [aten.embedding_dense_backward]
            stream2 = get_raw_stream(2)
            triton_poi_fused_embedding_dense_backward_12.run(buf43, 25165824, stream=stream2)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_42, unsqueeze_4, expand, convert_element_type_default, eq, unsqueeze_5, full_default_3, where, full_default_4, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream2 = get_raw_stream(2)
            ...
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream2 = get_raw_stream(2)
            triton_poi_fused_embedding_dense_backward_14.run(buf43, buf45, 25165824, stream=stream2)
            del buf43
