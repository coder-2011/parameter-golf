...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/46/c46zxucs2lpfedmq7bhrxd4ybvml5gzgd4khkbj7nymk2hy5s5nj.py
...
# Graph fragment:
#   %full_default_4 : Tensor "f32[393216, 128][128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([393216, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:1, pin_memory: False})
#   return %index_put
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/x3/cx3sogtzrj2edn46rsohibctihqxyqw3uqf3z24bgt3uzkpzjl42.py
...
#   %index_put : Tensor "f32[393216, 128][128, 1]cuda:1" = PlaceHolder[target=index_put]
...
#   %mul_42 : Tensor "f32[128, 1024, 128][131072, 128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_75, 0.28867513459481287), kwargs = {})
#   %unsqueeze_4 : Tensor "f32[128, 1024, 1, 128][131072, 128, 128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%mul_42, -2), kwargs = {})
#   %expand : Tensor "f32[128, 1024, 12, 128][131072, 128, 0, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%unsqueeze_4, [128, 1024, 12, 128]), kwargs = {})
#   %convert_element_type_default : Tensor "f32[128, 1024, 12, 128][1572864, 1536, 128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%expand, torch.float32), kwargs = {})
#   %eq : Tensor "b8[128, 1024, 12][12288, 12, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.eq.Scalar](args = (%primals_1, -1), kwargs = {})
#   %unsqueeze_5 : Tensor "b8[128, 1024, 12, 1][12288, 12, 1, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%eq, -1), kwargs = {})
...
#   %where : Tensor "f32[128, 1024, 12, 128][1572864, 1536, 128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.where.self](args = (%unsqueeze_5, %full_default_3, %convert_element_type_default), kwargs = {})
#   %full_default_4 : Tensor "f32[393216, 128][128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([393216, 128], 0), kwargs = {dtype: torch.float32, layout: torch.strided, device: cuda:1, pin_memory: False})
#   %index_put : Tensor "f32[393216, 128][128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.aten.index_put_.default](args = (%full_default_4, [%primals_1], %where, True), kwargs = {})
#   return %buf44
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/eu/ceuohetaapjcxg3atxzcu3fjmjzcjoku56xfaitujmhrdo4o7uee.py
...
# Graph fragment:
#   %buf44 : Tensor "f32[393216, 128][128, 1]cuda:1" = PlaceHolder[target=buf44]
#   %convert_element_type_80 : Tensor "bf16[393216, 128][128, 1]cuda:1"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%index_put, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(1):
            ...
            # Topologically Sorted Source Nodes: [full_default_4], Original ATen: [aten.embedding_dense_backward]
            stream1 = get_raw_stream(1)
            triton_poi_fused_embedding_dense_backward_12.run(buf43, 50331648, stream=stream1)
            # Topologically Sorted Source Nodes: [view_30, convert_element_type_75, mul_42, unsqueeze_4, expand, convert_element_type_default, eq, unsqueeze_5, full_default_3, where, full_default_4, index_put], Original ATen: [aten.view, aten._to_copy, aten.mul, aten.unsqueeze, aten.expand, aten.embedding_dense_backward]
            stream1 = get_raw_stream(1)
            triton_poi_fused__to_copy_embedding_dense_backward_expand_mul_unsqueeze_view_13.run(primals_1, buf42, buf43, 1572864, 128, stream=stream1)
            del buf42
            del primals_1
            buf45 = reinterpret_tensor(buf40, (393216, 128), (128, 1), 0); del buf40  # reuse
            # Topologically Sorted Source Nodes: [convert_element_type_80], Original ATen: [aten.embedding_dense_backward]
            stream1 = get_raw_stream(1)
            triton_poi_fused_embedding_dense_backward_14.run(buf43, buf45, 50331648, stream=stream1)
            del buf43
