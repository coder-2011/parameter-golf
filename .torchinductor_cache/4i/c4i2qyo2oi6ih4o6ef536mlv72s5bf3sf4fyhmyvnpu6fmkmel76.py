...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/3p/c3p7w65z5lawks277ksehvmi4jo2goap5l57g3vxbzdszkchlkmh.py
# Topologically Sorted Source Nodes: [zeros], Original ATen: [aten.zeros]
# Source node to ATen node mapping:
#   zeros => full_default
# Graph fragment:
#   %full_default : Tensor "bf16[512, 1000, 384][384000, 384, 1]cuda:7"[num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([512, 1000, 384], 0), kwargs = {dtype: torch.bfloat16, layout: torch.strided, device: cuda:7, pin_memory: False})
#   return %full_default
...
class Runner:
    ...
    def call(self, args):
        with torch.cuda._DeviceGuard(7):
            ...
            triton_poi_fused_zeros_0.run(buf0, 196608000, stream=stream7)
        return (buf0, )
