...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/nk/cnk5o6jqqtonlkshcpy34p3evrhwrri22m7ytsrcrgp74qrqxde2.py
...
# Graph fragment:
#   %primals_1 : Tensor "bf16[384, 384][384, 1]cuda:2" = PlaceHolder[target=primals_1]
#   %convert_element_type_default_14 : Tensor "bf16[384, 384][384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_1, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[384, 384][1, 384]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_14, [1, 0]), kwargs = {})
#   return %permute
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/5m/c5mtz5baq5nw6oviap6457p35appjnxcs3vjf7ptmiv37u3ybprp.py
...
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_2, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/vb/cvbobds74ta5wtbxx4srpng7hgqbabj237thivgdsjyght2j7vex.py
...
#   %buf3 : Tensor "f32[128, 1024, 1][1024, 1, 131072]cuda:2" = PlaceHolder[target=buf3]
#   %rsqrt : Tensor "f32[128, 1024, 1][1024, 1, 1]cuda:2" = PlaceHolder[target=rsqrt]
#   %primals_3 : Tensor "f32[384][1]cuda:2" = PlaceHolder[target=primals_3]
...
#   %convert_element_type_5 : Tensor "bf16[384][1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_3, torch.bfloat16), kwargs = {})
#   %convert_element_type_6 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_1, torch.float32), kwargs = {})
#   %pow_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%convert_element_type_6, 2), kwargs = {})
...
#   %add : Tensor "f32[128, 1024, 1][1024, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.add.Scalar](args = (%mean, 1e-06), kwargs = {})
#   %rsqrt : Tensor "f32[128, 1024, 1][1024, 1, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add,), kwargs = {})
#   %mul : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_6, %rsqrt), kwargs = {})
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%mul, %convert_element_type_5), kwargs = {})
#   %convert_element_type_7 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_1, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/ek/cek64brohdlamdsqict4ag2yucbdmtte6fahsjo3tj7amuzan5y4.py
...
#   %clone : Tensor "bf16[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2" = PlaceHolder[target=clone]
#   %sum_1 : Tensor "f32[128, 1024, 4, 1][4096, 4, 1, 524288]cuda:2" = PlaceHolder[target=sum_1]
#   %pow_3 : Tensor "f32[128, 1024, 4, 1][4096, 4, 1, 1]cuda:2" = PlaceHolder[target=pow_3]
#   %view_3 : Tensor "bf16[128, 1024, 768][786432, 768, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mm_1, [128, 1024, 768]), kwargs = {})
#   %view_4 : Tensor "bf16[128, 1024, 16, 48][786432, 768, 48, 1]cuda:2"[num_users=3] = call_function[target=torch.ops.aten.reshape.default](args = (%view_3, [128, 1024, 16, 48]), kwargs = {})
#   %slice_1 : Tensor "bf16[128, 1024, 4, 48][786432, 768, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%view_4, 2, 12, 9223372036854775807), kwargs = {})
#   %clone : Tensor "bf16[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2"[num_users=4] = call_function[target=torch.ops.aten.clone.default](args = (%slice_1,), kwargs = {memory_format: torch.contiguous_format})
#   %convert_element_type_10 : Tensor "f32[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%clone, torch.float32), kwargs = {})
#   %pow_2 : Tensor "f32[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%convert_element_type_10, 2.0), kwargs = {})
#   %sum_1 : Tensor "f32[128, 1024, 4, 1][4096, 4, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%pow_2, [-1], True), kwargs = {})
#   %pow_3 : Tensor "f32[128, 1024, 4, 1][4096, 4, 1, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%sum_1, 0.5), kwargs = {})
#   %clamp_min : Tensor "f32[128, 1024, 4, 1][4096, 4, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.clamp_min.default](args = (%pow_3, 1e-12), kwargs = {})
#   %expand : Tensor "f32[128, 1024, 4, 48][4096, 4, 1, 0]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.expand.default](args = (%clamp_min, [128, 1024, 4, 48]), kwargs = {})
#   %div : Tensor "f32[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.div.Tensor](args = (%clone, %expand), kwargs = {})
...
@triton_heuristics.persistent_reduction(
    ...
    reduction_hint=ReductionHint.INNER,
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr0': '*fp32', 'in_ptr0': '*bf16', 'out_ptr0': '*bf16', 'out_ptr1': '*fp32', 'xnumel': 'i32', 'r0_numel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=2, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]], (4,): [['tt.divisibility', 16]], (5,): [['tt.divisibility', 16]]}]},
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/q4/cq4er6fji7yyrpum6mlvn52uuz5ulsyjjrvcktqy6pk4n6w5tx6c.py
...
#   %div : Tensor "f32[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2" = PlaceHolder[target=div]
#   %permute_7 : Tensor "bf16[128, 1024, 8, 48][393216, 384, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%getitem_2, [0, 2, 1, 3]), kwargs = {})
#   %view_5 : Tensor "bf16[128, 1024, 4, 2, 48][393216, 384, 96, 48, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.reshape.default](args = (%permute_7, [128, 1024, 4, 2, 48]), kwargs = {})
#   %unsqueeze : Tensor "f32[128, 1024, 4, 1, 48][196608, 192, 48, 48, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%div, -2), kwargs = {})
#   %mul_2 : Tensor "f32[128, 1024, 4, 2, 48][393216, 384, 96, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view_5, %unsqueeze), kwargs = {})
#   %sum_2 : Tensor "f32[128, 1024, 4, 2, 1][8192, 8, 2, 1, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%mul_2, [-1], True), kwargs = {dtype: torch.float32})
#   return %sum_2
...
# Graph fragment:
#   %primals_7 : Tensor "f32[8, 12][12, 1]cuda:2" = PlaceHolder[target=primals_7]
#   %convert_element_type_11 : Tensor "bf16[8, 12][12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_7, torch.bfloat16), kwargs = {})
#   %permute_8 : Tensor "bf16[12, 8][1, 12]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_11, [1, 0]), kwargs = {})
#   return %permute_8
...
#   getitem_3 => slice_2
# Graph fragment:
#   %convert_element_type_7 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2" = PlaceHolder[target=convert_element_type_7]
#   %slice_2 : Tensor "bf16[128, 1024, 12][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.slice.Tensor](args = (%convert_element_type_7, 2, 0, 12), kwargs = {})
#   %clone_1 : Tensor "bf16[128, 1024, 12][12288, 12, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.clone.default](args = (%slice_2,), kwargs = {memory_format: torch.contiguous_format})
#   return %clone_1
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/6r/c6r56ri3hbqzvqycsfvismxtk3m2ohfiyhpkscglky6zo3junhqs.py
...
# Graph fragment:
#   %getitem_2 : Tensor "bf16[128, 8, 1024, 48][393216, 48, 384, 1]cuda:2" = PlaceHolder[target=getitem_2]
#   %sum_2 : Tensor "f32[128, 1024, 4, 2, 1][8192, 8, 2, 1, 1]cuda:2" = PlaceHolder[target=sum_2]
#   %div : Tensor "f32[128, 1024, 4, 48][196608, 192, 48, 1]cuda:2" = PlaceHolder[target=div]
#   %mm_2 : Tensor "bf16[131072, 8][8, 1]cuda:2" = PlaceHolder[target=mm_2]
#   %permute_7 : Tensor "bf16[128, 1024, 8, 48][393216, 384, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.permute.default](args = (%getitem_2, [0, 2, 1, 3]), kwargs = {})
#   %view_5 : Tensor "bf16[128, 1024, 4, 2, 48][393216, 384, 96, 48, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.reshape.default](args = (%permute_7, [128, 1024, 4, 2, 48]), kwargs = {})
#   %unsqueeze : Tensor "f32[128, 1024, 4, 1, 48][196608, 192, 48, 48, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%div, -2), kwargs = {})
#   %mul_3 : Tensor "f32[128, 1024, 4, 2, 48][393216, 384, 96, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_2, %unsqueeze), kwargs = {})
#   %sub : Tensor "f32[128, 1024, 4, 2, 48][393216, 384, 96, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%view_5, %mul_3), kwargs = {})
#   %view_6 : Tensor "f32[128, 1024, 8, 48][393216, 384, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%sub, [128, 1024, 8, 48]), kwargs = {})
...
#   %mul_4 : Tensor "bf16[128, 1024, 8][8192, 8, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view_8, 0.5), kwargs = {})
#   %sigmoid : Tensor "bf16[128, 1024, 8][8192, 8, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.sigmoid.default](args = (%mul_4,), kwargs = {})
#   %unsqueeze_1 : Tensor "bf16[128, 1024, 8, 1][8192, 8, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.unsqueeze.default](args = (%sigmoid, 3), kwargs = {})
#   %mul_5 : Tensor "f32[128, 1024, 8, 48][393216, 384, 48, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%view_6, %unsqueeze_1), kwargs = {})
#   %view_9 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.reshape.default](args = (%mul_5, [128, 1024, 384]), kwargs = {})
#   %convert_element_type_16 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%view_9, torch.bfloat16), kwargs = {})
...
@triton_heuristics.pointwise(
    size_hints={'x': 67108864}, 
    filename=__file__,
    triton_meta={'signature': {'in_ptr0': '*bf16', 'in_ptr1': '*fp32', 'in_ptr2': '*fp32', 'in_ptr3': '*bf16', 'out_ptr0': '*bf16', 'xnumel': 'i32', 'XBLOCK': 'constexpr'}, 'device': DeviceProperties(type='cuda', index=2, multi_processor_count=132, cc=90, major=9, regs_per_multiprocessor=65536, max_threads_per_multi_processor=2048, max_threads_per_block=1024, warp_size=32), 'constants': {}, 'native_matmul': False, 'enable_fp_fusion': True, 'launch_pdl': False, 'disable_ftz': False, 'configs': [{(0,): [['tt.divisibility', 16]], (1,): [['tt.divisibility', 16]], (2,): [['tt.divisibility', 16]], (3,): [['tt.divisibility', 16]], (4,): [['tt.divisibility', 16]], (5,): [['tt.divisibility', 16]]}]},
...
#   %add_1 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=3] = call_function[target=torch.ops.aten.add.Tensor](args = (%view_11, %view_1), kwargs = {})
#   %convert_element_type_19 : Tensor "bf16[384][1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_9, torch.bfloat16), kwargs = {})
#   %convert_element_type_20 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%add_1, torch.float32), kwargs = {})
#   %pow_4 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.pow.Tensor_Scalar](args = (%convert_element_type_20, 2), kwargs = {})
#   %mean_1 : Tensor "f32[128, 1024, 1][1024, 1, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mean.dim](args = (%pow_4, [2], True), kwargs = {})
...
#   %rsqrt_1 : Tensor "f32[128, 1024, 1][1024, 1, 1]cuda:2"[num_users=2] = call_function[target=torch.ops.aten.rsqrt.default](args = (%add_2,), kwargs = {})
#   %mul_6 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%convert_element_type_20, %rsqrt_1), kwargs = {})
#   %mul_7 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%mul_6, %convert_element_type_19), kwargs = {})
#   %convert_element_type_21 : Tensor "bf16[128, 1024, 384][393216, 384, 1]cuda:2"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_7, torch.bfloat16), kwargs = {})
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(2):
            ...
            triton_poi_fused__to_copy_t_0.run(primals_1, buf0, 147456, stream=stream2)
            del primals_1
            buf1 = empty_strided_cuda((128, 1024, 384), (393216, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy]
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_1.run(primals_2, buf1, 50331648, stream=stream2)
            del primals_2
            ...
            # Topologically Sorted Source Nodes: [linear, to_1, rms_norm], Original ATen: [aten._unsafe_view, aten._to_copy, aten._fused_rms_norm]
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_2.run(buf4, buf2, primals_3, buf5, 131072, 384, stream=stream2)
            ...
            # Topologically Sorted Source Nodes: [linear_1, view, getitem, v, normalize], Original ATen: [aten._unsafe_view, aten.view, aten.slice, aten.clone, aten.linalg_vector_norm, aten.clamp_min, aten.expand, aten.div]
            stream2 = get_raw_stream(2)
            triton_per_fused__unsafe_view_clamp_min_clone_div_expand_linalg_vector_norm_slice_view_3.run(buf19, buf6, buf9, buf20, 524288, 48, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            _fused_qkv_postprocess_fwd_kernel_0.run(reinterpret_tensor(buf6, (128, 1024, 16, 48), (786432, 768, 48, 1), 0), buf7, buf8, reinterpret_tensor(primals_5, (1024, 24), (24, 1), 0), reinterpret_tensor(primals_6, (1024, 24), (24, 1), 0), 1572864, 1, 1, stream=stream2)
            ...
            # Topologically Sorted Source Nodes: [transpose_3, view_1, unsqueeze, mul, sum_1], Original ATen: [aten.transpose, aten.view, aten.unsqueeze, aten.mul, aten.sum]
            stream2 = get_raw_stream(2)
            triton_per_fused_mul_sum_transpose_unsqueeze_view_4.run(buf13, buf20, buf21, 1048576, 48, stream=stream2)
            buf22 = empty_strided_cuda((12, 8), (1, 12), torch.bfloat16)
            # Topologically Sorted Source Nodes: [to_3, linear_2], Original ATen: [aten._to_copy, aten.t]
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_5.run(primals_7, buf22, 96, stream=stream2)
            del primals_7
            buf23 = empty_strided_cuda((128, 1024, 12), (12288, 12, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [getitem_3, contiguous_4], Original ATen: [aten.slice, aten.clone]
            stream2 = get_raw_stream(2)
            triton_poi_fused_clone_slice_6.run(buf5, buf23, 1572864, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_0.run(primals_8, buf25, 147456, stream=stream2)
            del primals_8
            buf26 = empty_strided_cuda((128, 1024, 384), (393216, 384, 1), torch.bfloat16)
            # Topologically Sorted Source Nodes: [transpose_3, view_1, unsqueeze, mul_1, sub, reshape, linear_2, mul_2, sigmoid, getitem_4, mul_3, reshape_1, linear_3], Original ATen: [aten.transpose, aten.view, aten.unsqueeze, aten.mul, aten.sub, aten._unsafe_view, aten.sigmoid, aten._to_copy]
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy__unsafe_view_mul_sigmoid_sub_transpose_unsqueeze_view_7.run(buf13, buf21, buf20, buf24, buf26, 50331648, stream=stream2)
            ...
            # Topologically Sorted Source Nodes: [linear, linear_3, add, to_5, rms_norm_1], Original ATen: [aten._unsafe_view, aten.add, aten._to_copy, aten._fused_rms_norm]
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_8.run(buf28, buf30, buf2, primals_9, buf31, 131072, 384, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            _leaky_relu_sq_matmul_kernel_1.run(reinterpret_tensor(buf31, (131072, 384), (384, 1), 0), primals_10, buf32, buf33, 24576, 1, 1, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_8.run(buf37, buf39, buf28, primals_12, buf40, 131072, 384, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__unsafe_view_clamp_min_clone_div_expand_linalg_vector_norm_slice_view_3.run(buf54, buf41, buf44, buf55, 524288, 48, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            _fused_qkv_postprocess_fwd_kernel_0.run(reinterpret_tensor(buf41, (128, 1024, 16, 48), (786432, 768, 48, 1), 0), buf42, buf43, reinterpret_tensor(primals_5, (1024, 24), (24, 1), 0), reinterpret_tensor(primals_6, (1024, 24), (24, 1), 0), 1572864, 1, 1, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused_mul_sum_transpose_unsqueeze_view_4.run(buf48, buf55, buf56, 1048576, 48, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_5.run(primals_14, buf57, 96, stream=stream2)
            del primals_14
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused_clone_slice_6.run(buf40, buf58, 1572864, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_0.run(primals_15, buf60, 147456, stream=stream2)
            del primals_15
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy__unsafe_view_mul_sigmoid_sub_transpose_unsqueeze_view_7.run(buf48, buf56, buf55, buf59, buf61, 50331648, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_8.run(buf63, buf65, buf37, primals_16, buf66, 131072, 384, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            _leaky_relu_sq_matmul_kernel_1.run(reinterpret_tensor(buf66, (131072, 384), (384, 1), 0), primals_17, buf67, buf68, 24576, 1, 1, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_8.run(buf72, buf74, buf63, primals_19, buf75, 131072, 384, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__unsafe_view_clamp_min_clone_div_expand_linalg_vector_norm_slice_view_3.run(buf89, buf76, buf79, buf90, 524288, 48, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            _fused_qkv_postprocess_fwd_kernel_0.run(reinterpret_tensor(buf76, (128, 1024, 16, 48), (786432, 768, 48, 1), 0), buf77, buf78, reinterpret_tensor(primals_5, (1024, 24), (24, 1), 0), reinterpret_tensor(primals_6, (1024, 24), (24, 1), 0), 1572864, 1, 1, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused_mul_sum_transpose_unsqueeze_view_4.run(buf83, buf90, buf91, 1048576, 48, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_5.run(primals_21, buf92, 96, stream=stream2)
            del primals_21
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused_clone_slice_6.run(buf75, buf93, 1572864, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy_t_0.run(primals_22, buf95, 147456, stream=stream2)
            del primals_22
            ...
            stream2 = get_raw_stream(2)
            triton_poi_fused__to_copy__unsafe_view_mul_sigmoid_sub_transpose_unsqueeze_view_7.run(buf83, buf91, buf90, buf94, buf96, 50331648, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_8.run(buf98, buf100, buf72, primals_23, buf101, 131072, 384, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            _leaky_relu_sq_matmul_kernel_1.run(reinterpret_tensor(buf101, (131072, 384), (384, 1), 0), primals_24, buf102, buf103, 24576, 1, 1, stream=stream2)
            ...
            stream2 = get_raw_stream(2)
            triton_per_fused__fused_rms_norm__to_copy__unsafe_view_add_8.run(buf107, buf109, buf98, primals_26, buf110, 131072, 384, stream=stream2)
...
def get_args():
    ...
    primals_1 = rand_strided((384, 384), (384, 1), device='cuda:2', dtype=torch.bfloat16)
    ...
    primals_5 = rand_strided((1, 1024, 1, 24), (24576, 24, 24, 1), device='cuda:2', dtype=torch.bfloat16)
    primals_6 = rand_strided((1, 1024, 1, 24), (24576, 24, 24, 1), device='cuda:2', dtype=torch.bfloat16)
    primals_7 = rand_strided((8, 12), (12, 1), device='cuda:2', dtype=torch.float32)
    primals_8 = rand_strided((384, 384), (384, 1), device='cuda:2', dtype=torch.bfloat16)
    ...
    primals_21 = rand_strided((8, 12), (12, 1), device='cuda:2', dtype=torch.float32)
    primals_22 = rand_strided((384, 384), (384, 1), device='cuda:2', dtype=torch.bfloat16)
    ...
    primals_27 = rand_strided((128, 1024), (1024, 1), device='cuda:2', dtype=torch.int64)
