...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/w5/cw5qpdcy6ihsp5nsaseuzulbpuqb3zel2q7siaxy6doj4s6lslcp.py
...
#   %exp_1 : Tensor "f32[384][1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.exp.default](args = (%primals_2,), kwargs = {})
...
#   %mul_1 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%primals_3, %exp_2), kwargs = {})
...
#   %mul_2 : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%where, %view_1), kwargs = {})
#   %add : Tensor "f32[128, 1024, 384][393216, 384, 1]cuda:5"[num_users=1] = call_function[target=torch.ops.aten.add.Tensor](args = (%mul_1, %mul_2), kwargs = {})
#   return %add
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(5):
            ...
            triton_poi_fused__unsafe_view_add_exp_mul_neg_softplus_0.run(primals_3, primals_1, primals_2, buf0, buf1, 50331648, stream=stream5)
