...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/hk/chkau4ryezz4bjzyepozaixtj2v5ty36ccvd5ibqbojlzwmp3ogf.py
...
#   %primals_7 : Tensor "bf16[65536, 112][112, 1]cuda:4" = PlaceHolder[target=primals_7]
#   %sum_1 : Tensor "f32[128, 1024, 112][114688, 112, 1]cuda:4" = PlaceHolder[target=sum_1]
#   %embedding_1 : Tensor "bf16[128, 1024, 8, 112][917504, 896, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.embedding.default](args = (%primals_7, %add_1), kwargs = {})
#   %sum_1 : Tensor "f32[128, 1024, 112][114688, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.sum.dim_IntList](args = (%embedding_1, [-2]), kwargs = {dtype: torch.float32})
#   %mul_3 : Tensor "f32[128, 1024, 112][114688, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.aten.mul.Tensor](args = (%sum_1, 0.3535533905932738), kwargs = {})
#   %convert_element_type_2 : Tensor "bf16[128, 1024, 112][114688, 112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%mul_3, torch.bfloat16), kwargs = {})
...
# kernel path: /workspace/parameter-golf/.torchinductor_cache/7n/c7ndrjnzdmakshrqbugzns4tpvadfmy7bejkvqd2td6hhxggsdif.py
# Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
# Source node to ATen node mapping:
#   linear => convert_element_type_default_7, permute
# Graph fragment:
#   %primals_8 : Tensor "bf16[384, 112][112, 1]cuda:4" = PlaceHolder[target=primals_8]
#   %convert_element_type_default_7 : Tensor "bf16[384, 112][112, 1]cuda:4"[num_users=1] = call_function[target=torch.ops.prims.convert_element_type.default](args = (%primals_8, torch.bfloat16), kwargs = {})
#   %permute : Tensor "bf16[112, 384][1, 112]cuda:4"[num_users=2] = call_function[target=torch.ops.aten.permute.default](args = (%convert_element_type_default_7, [1, 0]), kwargs = {})
#   return %permute
...
class Runner:
    ...
    def call(self, args):
        ...
        with torch.cuda._DeviceGuard(4):
            ...
            # Topologically Sorted Source Nodes: [embedding_1, sum_1, mul_3, linear], Original ATen: [aten.embedding, aten.sum, aten.mul, aten._to_copy]
            stream4 = get_raw_stream(4)
            triton_per_fused__to_copy_embedding_mul_sum_3.run(buf3, primals_7, buf6, 14680064, 8, stream=stream4)
            del primals_7
            buf5 = empty_strided_cuda((112, 384), (1, 112), torch.bfloat16)
            # Topologically Sorted Source Nodes: [linear], Original ATen: [aten._to_copy, aten.t]
            stream4 = get_raw_stream(4)
            triton_poi_fused__to_copy_t_4.run(primals_8, buf5, 43008, stream=stream4)
            del primals_8
...
def get_args():
    ...
    primals_7 = rand_strided((65536, 112), (112, 1), device='cuda:4', dtype=torch.bfloat16)
    primals_8 = rand_strided((384, 112), (112, 1), device='cuda:4', dtype=torch.bfloat16)
