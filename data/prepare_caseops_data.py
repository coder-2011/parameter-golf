...
def main() -> None:
    ...
    ap.add_argument("--train-shards", type=int, default=0, help="Stop after this many train shards; 0 means all")
    args = ap.parse_args()
    ...
    for text in _iter_docs(args.docs):
        ...
        if n_docs < args.val_docs:
            ...
            if len(train_buf) >= SHARD_TOKENS:
                ...
                train_written += 1
                if args.train_shards and train_written >= args.train_shards:
                    break
        n_docs += 1
        if n_docs % 10_000 == 0:
