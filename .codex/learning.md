...
- 2026-04-28: Parcae delayed QAT no longer computes `self.step >= QAT_START_STEP` inside `forward_model`. The training loop calls `GPT.set_training_step(step)`, which latches QAT mode and updates `CastedLinear.qat_enabled` outside compiled forward. This avoids `torch.compile` guarding on the per-step Python integer when `COMPILE_MODEL=1`; delayed QAT should now require at most a mode-change graph, not one graph per step. Warmup also primes a QAT-on step when `QAT_START_STEP` is beyond the normal warmup range, so the mode-change graph is not first compiled during measured training.
...
- 2026-04-28: `train_gpt_parcae.py` has a default-off Triton RMSNorm path controlled by `TRITON_RMSNORM=1`. The supplied external kernel's backward formula was wrong; the implemented version uses `dot=sum(dy*weight*x)` and fp32 partial `dw` accumulation. Tiny CUDA train step passed. Microbench versus current `ParcaeRMSNorm` showed large speedups (`~2.2x` forward and `~1.6x` backward for 4096x64/512 bf16), but it is not exact to current cast order: `dx` is close (`<=0.002`) while `dw` differs by about `0.06-0.13` absolute in synthetic tests. Keep default off until a same-seed quality smoke is run.
