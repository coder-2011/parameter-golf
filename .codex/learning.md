...
- 2026-04-30: Parcae fused QKV postprocess was tuned for the active width-416 shape (`Hq=8,Hkv=4,D=52,rope=32`). The Triton kernel now handles only Q/K heads; V is taken as a contiguous slice and copied in backward, avoiding wasted RoPE/RMSNorm work on V heads. Inactive Q/K pointer bases are clamped before masked loads/stores to avoid large-shape Triton instability. Focused CUDA checks passed for D52 (`q/k/v allclose`, fwd+bwd), and a tiny width-416 GPT fwd/bwd passed with `FUSED_QKV_POSTPROCESS=1 LIGER_ROPE=0`. Contended microbench showed about `1.66x` forward and `1.31x` fwd+bwd speedup over the Python postprocess path; the live autoresearch run `autoresearch_20260430_025650_611196` is testing model-level impact.
