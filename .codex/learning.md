...
- 2026-04-30: Parameter Golf timing rule audit: GPTQ/Hessian calibration must count against the training budget, not the eval budget or an unmeasured artifact-construction gap. `train_gpt_parcae.py` now keeps `GPTQ_RESERVE_SECONDS` only as a loop-stop reserve, measures actual GPTQ collection+quantization time, and errors if `train_loop + actual_gptq` exceeds `MAX_WALLCLOCK_SECONDS`. Score-first TTT is now run after quantized roundtrip eval so validation-token adaptation cannot affect artifact export.

- 2026-04-30: `train_gpt_parcae.py` has an opt-in `FUSED_QKV_POSTPROCESS=1` Triton path for packed QKV postprocessing. It targets the common CUDA/no-bias/no-clip/no-value-gate shape and fuses adjacent-pair RoPE, optional QK RMSNorm, and contiguous Q/K/V materialization with a custom backward. Local RTX PRO 4500 Blackwell smokes passed: direct postprocess forward/backward parity, full attention output/gradient parity, and tiny GPT forward/backward. Microbench on `B=8,T=512,Hq=8,Hkv=4,D=64` showed about `1.41x` postprocess forward and `1.12x` postprocess forward+backward speedup over TriDao RoPE + RMSNorm + contiguous; full attention `B=4,T=256,D=512` forward+backward showed about `1.15x`.
...
- 2026-04-30: Parcae grouped quant artifacts are lossless and now use a custom `PGQG1` container. Tensor metadata lives in a compressed JSON manifest; payloads are grouped by tensor family, multi-byte floating tensors are split into global byte-lane groups, and each group independently picks the smallest available codec among store/zlib/lzma/brotli/zstd. On `final_model.pt`, the raw-state probe round-tripped exactly and reduced `torch.save+zlib` from 35,164,397 bytes to 31,579,643 bytes; the default int6 quant probe reduced 7,122,042 bytes to 6,947,422 bytes with `roundtrip_equal=1`.

- 2026-04-30: New Parcae 300s dense baseline beat the prior non-PWA exact result. Config highlights: 384d, recurrent 384, 8H/4KV, packed dense QKV, Base SOTA stack plus `FusedLeakyReLUSqMLP`, sparse attention gate, `EVAL_SEQ_LEN=1000`, grouped int6 rANS artifact, `TTT_ENABLED=0`. It reached step 1543, exact `val_bpb=1.39797022`, submission `9,507,070` bytes, `13,682,977` params. Prior non-PWA exact was `1.39905976` and `9,541,430` bytes. Recent PWA Q/K dense-V trials were smaller but materially worse in BPB, so keep dense packed QKV as the SOTA path for now.

- 2026-04-30: A 600s Parcae hash/Muon trial with `BIGRAM_HASH_HEADS=8`, `BIGRAM_HASH_GATE=1`, `MUON_WD=0.105`, `TTT_ENABLED=0`, and the same dense sparse-gate/FusedLeakyReLUSq stack reached step 3026, exact `val_bpb=1.36637055`, submission `12,473,075` bytes, `17,877,281` params. It improves BPB at 600s but costs about 4.19M extra params and 2.97MB extra submission versus the 300s 4-head baseline, so this is not yet evidence that 8 hash heads is parameter-efficient.

- 2026-04-30: AutoKernel Parcae setup uses the external `scripts/autokernel_parcae_adapter.py` and direct Python by default because uv is not compatible with this AutoKernel checkout in the local loop. Normalizing profiler shape strings before extraction fixed the starter-shape fallback warning. Top extracted kernels from a tiny `B=2,T=128` Parcae profile: matmul ranks 2/5/6, RMS/layernorm rank 4, flash attention rank 9. Isolated bests were matmul `64x128x64` with fp32 fallback, layernorm `BLOCK_ROWS=2`/single-warp maskless short rows, and flash attention exact-primary mask removal (`4.274 TFLOPS` best single sample, noisy). Upstream `verify.py` only applies generic `nn.Linear`/`nn.LayerNorm` wrappers; Parcae custom `ParcaeRMSNorm` and `F.scaled_dot_product_attention`/flash-attn paths need a Parcae-specific integration adapter before model-level speedup numbers are meaningful.

- 2026-04-30: Parcae fused QKV postprocess was tuned for the active width-416 shape (`Hq=8,Hkv=4,D=52,rope=32`). The Triton kernel now handles only Q/K heads; V is taken as a contiguous slice and copied in backward, avoiding wasted RoPE/RMSNorm work on V heads. Inactive Q/K pointer bases are clamped before masked loads/stores to avoid large-shape Triton instability. It supports both adjacent-pair RoPE (`LIGER_ROPE=0`) and Liger split-half RoPE (`LIGER_ROPE=1`) so it can be a drop-in opt-in. Focused CUDA checks passed for D52 and width-416 GPT fwd/bwd. However, against the actual current Liger path, the equivalent fused attention fwd+bwd microbench was slower (`3.78ms` fused vs `2.39ms` Liger), so `autoresearch.sh` keeps `FUSED_QKV_POSTPROCESS=0` for now. The fused path is useful for non-Liger/Python-RoPE experiments, not current SOTA defaults.

- 2026-04-30: `scripts/launch_caseops_8h100.sh` is now the dedicated single-node 8xH100 SXM launcher. It asserts eight H100-class ~80GB GPUs by default, checks `/workspace` free space, logs topology/NVLink status, persists TorchInductor cache under the repo, keeps `COMPILE_MODEL=1`, leaves `COMPILE_MUON_BACKEND=0` by default because that path previously caused CUDA illegal-memory faults, and exposes `PROFILE=smoke|balanced|throughput|maxmem` to probe per-rank batch scaling before full tuning. Local validation passed with `bash -n`, expected 8-GPU preflight failure on the 1x RTX PRO 4500 box, and `DRY_RUN=1` profile resolution.

- 2026-04-30: This workstation exposes a Blackwell `sm_120` GPU, but the installed PyTorch is `2.4.1+cu124` and only advertises support through `sm_90`; CUDA pytest cases fail with `no kernel image is available for execution on the device`. Use CPU-focused checks with `CUDA_VISIBLE_DEVICES=` here, or install a torch build with `sm_120` support before trusting CUDA/Triton smoke results.

- 2026-04-30: The PR-1797-style port into `train_gpt_parcae.py` keeps record features opt-in where they alter training semantics: `FUSED_SOFTCAPPED_CE`, `DOCUMENT_PACKING`, `ATTN_OUT_GATE_ENABLED`, `GATED_ATTN_ENABLED`, `LQER_ENABLED`, `MUON_NS_COEFFS=polar_express`, and `GPTQ_CALIBRATION_MODE=shuffled`. Defaults preserve the existing Parcae run unless a feature is explicitly enabled.

- 2026-04-30: For Blackwell CUDA checks in this repo, use the uv-managed venv rather than system Python. `.venv` is locked to `torch==2.11.0+cu130` and advertises `sm_120`; system `/usr/bin/python` has `torch==2.4.1+cu124` and fails CUDA kernels on this GPU. `uv` is available as `python -m uv` here. The locked venv does not include pytest, so focused tests can be run with `python -m uv run --no-sync --with pytest pytest ...`.

- 2026-04-30: Parcae residual forget gate is default-on via `RESIDUAL_FORGET_GATE=1`. Each transformer block gets an RG-LRU-style per-channel decay `exp(-8 * sigmoid(gate(x)) * softplus(a_param))` applied to the residual stream before adding attention/MLP branches. The gate starts near identity with zero gate weights and `RESIDUAL_FORGET_GATE_BIAS=-4.0`; explicit `RESIDUAL_FORGET_GATE=0` preserves the old residual arithmetic path, including delayed parallel residual equivalence.

- 2026-04-30: `autoresearch.sh` now explicitly exposes the recent Parcae experiment knobs instead of relying on hidden `train_gpt_parcae.py` defaults: residual forget, U-Net skips, immediate/delayed parallel residual controls, attention output/gated/sparse gates, fused softcapped CE, document packing, GPTQ calibration mode, LQER, mixed-quant bit maps, PWA/RRHP QKV controls, and Muon NS coefficients. `autoresearch.md` includes a feature glossary and the active CaseOps/SP8192 config, so agents should update both files when changing the active harness.
