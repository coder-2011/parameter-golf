...
class Hyperparameters:
    ...
    ppm_subset_tokens = int(os.environ.get('PPM_SUBSET_TOKENS', 5000000))
    ...
    gptq_reserve_seconds = float(os.environ.get('GPTQ_RESERVE_SECONDS', 2.0))
...
class ValidationData:
    ...
    def __init__(self, h: Hyperparameters, device: torch.device):
        ...
        if getattr(h, 'tokenizer_kind', 'sp') == 'scylla':
            self.sp = None
            self.base_bytes_lut, self.has_leading_space_lut, self.is_boundary_token_lut = build_scylla_luts(h.tokenizer_meta_path, h.vocab_size, device)
            self.token_bytes_py: list[bytes] = build_scylla_token_bytes_lut(h.tokenizer_path, h.vocab_size)
        else:
...
def build_scylla_luts(meta_path: str, vocab_size: int, device: torch.device) -> tuple[Tensor, Tensor, Tensor]:
    ...
    return (torch.tensor(bb, dtype=torch.int16, device=device), torch.tensor(hls, dtype=torch.bool, device=device), torch.tensor(isb, dtype=torch.bool, device=device))
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, rope_base: float, qk_gain_init: float, train_seq_len: int, gated_attn: bool=False, gated_attn_init_std: float=0.01):
        super().__init__()
        if dim % num_heads != 0:
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int, rope_base: float, qk_gain_init: float, train_seq_len: int, layer_idx: int=0, ln_scale: bool=False, parallel_residual: bool=False, gated_attn: bool=False, gated_attn_init_std: float=0.01):
        super().__init__()
        self.attn_norm = RMSNorm()
        self.mlp_norm = RMSNorm()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, train_seq_len, gated_attn=gated_attn, gated_attn_init_std=gated_attn_init_std)
...
class GPT(nn.Module):
    ...
    def __init__(self, h: Hyperparameters):
        ...
        self.blocks = nn.ModuleList([Block(h.model_dim, h.num_heads, h.num_kv_heads, h.mlp_mult, h.rope_base, h.qk_gain_init, h.train_seq_len, layer_idx=i, ln_scale=h.ln_scale, parallel_residual=i >= h.parallel_residual_start, gated_attn=h.gated_attn_enabled, gated_attn_init_std=h.gated_attn_init_std) for i in range(h.num_layers)])
        if h.rope_dims > 0:
        ...
        if self.mtp_heads > 0:
            self.mtp_projs = nn.ModuleList([CastedLinear(h.embedding_dim, h.embedding_dim, bias=False) for _ in range(self.mtp_heads)])
        else:
            self.mtp_projs = None
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        primary_loss = F.cross_entropy(primary_logits.reshape(-1, primary_logits.size(-1)).float(), target_ids.reshape(-1), reduction='mean')
        ...
        if self.mtp_projs is not None:
            for k, proj in enumerate(self.mtp_projs):
                ...
                aux_loss = F.cross_entropy(aux_logits.reshape(-1, aux_logits.size(-1)).float(), aux_targets.reshape(-1), reduction='mean')
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                curr += p.numel()
        return loss
CONTROL_TENSOR_NAME_PATTERNS = tuple((pattern for pattern in os.environ.get('CONTROL_TENSOR_NAME_PATTERNS', 'attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,attn_gate_w,attn_gate').split(',') if pattern))
...
def collect_hessians(model: nn.Module, train_loader: ShuffledSequenceLoader, h: Hyperparameters, device: torch.device, n_calibration_batches: int=64) -> dict[str, Tensor]:
...
def gptq_quantize_weight(w: Tensor, H: Tensor, clip_sigmas: float=3.0, clip_range: int=63, block_size: int=128) -> tuple[Tensor, Tensor]:
...
def gptq_mixed_quantize(state_dict: dict[str, Tensor], hessians: dict[str, Tensor], h: Hyperparameters) -> tuple[dict[str, Tensor], dict[str, object]]:
    ...
    for name, tensor in state_dict.items():
        ...
        if 'attn_gate_w' in name and t.is_floating_point() and (t.ndim == 2):
            ...
            sf = s.float().view(-1, 1)
            q4 = torch.round(t / sf).clamp(-7, 7).to(torch.int8)
            rows, cols = q4.shape
            assert cols % 2 == 0, 'attn_gate_w expects even column count for int4 packing'
            even = q4[:, 0::2].to(torch.int32) & 15
            odd = q4[:, 1::2].to(torch.int32) & 15
...
def serialize(h: Hyperparameters, base_model: torch.nn.Module, code: str) -> tuple[int, int]:
    ...
    sd_cpu = {k: v.detach().cpu() for k, v in base_model.state_dict().items() if not k.startswith('mtp_projs.') and (not k.startswith('jepa_predictor.'))}
...
def eval_val(h: Hyperparameters, device: torch.device, val_data: ValidationData, model: nn.Module) -> tuple[float, float]:
...
def _ppm_mixture_bpb(target_ids: np.ndarray, prev_ids: np.ndarray, nll_nats: np.ndarray, token_bytes_lut: list[bytes], has_leading_space_lut_np: np.ndarray, is_boundary_token_lut_np: np.ndarray, order: int=5, lambda_hi: float=0.9, lambda_lo: float=0.05, conf_threshold: float=0.9, token_order: int=0, use_meta_mix: bool=False, meta_alpha: float=0.995, meta_eta: float=2.0, meta_warmup_bytes: int=4096, log_prefix: str='ppm_mix') -> tuple[float, float, float]:
    _ln = math.log
    LOG2 = _ln(2.0)
    UNIFORM_LOGP = _ln(1.0 / 256.0)
    ...
    log(f'{log_prefix} bytes={total_bytes} mix_bpb={mix_bpb:.6f} ppm_only={ppm_bpb:.6f} nn_only={nn_bpb:.6f}{extra} mode={mode}')
    return (mix_bpb, ppm_bpb, nn_bpb)

def eval_val_sliding(h: Hyperparameters, device: torch.device, val_data: ValidationData, base_model: nn.Module, batch_seqs: int=32) -> tuple[float, float]:
    base_model.eval()
    ...
    if collect_ppm and (not dist.is_available() or not dist.is_initialized() or h.rank == 0):
        ...
        try:
            t0 = time.perf_counter()
            mix_bpb, ppm_bpb, nn_bpb = _ppm_mixture_bpb(target_ids=tgt_np, prev_ids=prev_np, nll_nats=nll_np, token_bytes_lut=val_data.token_bytes_py, has_leading_space_lut_np=has_leading_np, is_boundary_token_lut_np=is_boundary_np, order=h.ppm_order, lambda_hi=h.ppm_lambda_hi, lambda_lo=h.ppm_lambda_lo, conf_threshold=h.ppm_conf_threshold, token_order=h.ppm_token_order if h.ppm_use_meta_mix else 0, use_meta_mix=h.ppm_use_meta_mix, meta_alpha=h.ppm_meta_alpha, meta_eta=h.ppm_meta_eta, meta_warmup_bytes=h.ppm_meta_warmup_bytes)
...
def _install_lora_adapters(model: nn.Module, unfrozen_block_ids: list[int], rank: int, alpha: float, device: torch.device) -> list[Tensor]:
            ...
            if isinstance(mod, CastedLinear) and mod.weight.numel() >= 1024:
                ...
                def make_fwd(orig_fwd, A_param, B_param, s):
                    ...
                    def new_forward(x: Tensor) -> Tensor:
                        base = orig_fwd(x)
                        lora_out = F.linear(F.linear(x, A_param.to(dtype=x.dtype)), B_param.to(dtype=x.dtype))
        ...
        for mod_name, mod in block.named_modules():
            if hasattr(mod, '_orig_forward'):
                ...
                if hasattr(mod, 'lora_scale'):
                    delattr(mod, 'lora_scale')

def eval_val_sliding_ttt(h: Hyperparameters, device: torch.device, val_data: ValidationData, base_model: nn.Module, batch_seqs: int=32) -> tuple[float, float]:
    seq_len = h.eval_seq_len
    total_tokens = val_data.val_tokens.numel() - 1
    context_size = seq_len - h.eval_stride
    ...
    if use_lora:
        ...
        ttt_params = _install_lora_adapters(base_model, unfrozen_block_ids, h.ttt_lora_rank, h.ttt_lora_alpha, device)
        optimizer = torch.optim.AdamW(ttt_params, lr=h.ttt_lora_lr, betas=(0.9, 0.95), eps=1e-08, weight_decay=h.ttt_lora_wd)
    else:
...
def train_and_eval(h: Hyperparameters, device: torch.device) -> None:
    ...
    if h.ttt_enabled:
        if h.ttt_lora_rank > 0:
            log(f'ttt:enabled MODE=lora rank={h.ttt_lora_rank} alpha={h.ttt_lora_alpha} lr={h.ttt_lora_lr} wd={h.ttt_lora_wd} epochs={h.ttt_epochs} chunk={h.ttt_chunk} freeze_blocks={h.ttt_freeze_blocks}')
        else:
            log(f'ttt:enabled MODE=sgd lr={h.ttt_lr} momentum={h.ttt_momentum} epochs={h.ttt_epochs} chunk={h.ttt_chunk} freeze_blocks={h.ttt_freeze_blocks}')
        ...
        try:
            timed_eval('quantized_sliding_window_ttt', eval_val_sliding_ttt, h, device, val_data, eval_model)
        except Exception as e:
