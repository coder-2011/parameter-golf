...
def naive_softmax(x, dim=-1):
    """Direct baseline: max, exp, sum, divide."""
    shifted = x - x.max(dim=dim, keepdim=True).values
    numer = torch.exp(shifted)
    return numer / numer.sum(dim=dim, keepdim=True)
...
def run_quick_benchmarks():
    ...
    providers = [
        ("triton_pasted", lambda x: softmax(x, dim=-1)),
        ("torch_softmax", lambda x: pytorch_softmax(x, dim=-1)),
        ("naive", lambda x: naive_softmax(x, dim=-1)),
    ]
    ...
    for M, N in shapes:
        ...
        ref = pytorch_softmax(x, dim=-1)
        torch.cuda.synchronize()
        for name, fn in providers:
            try:
                y = fn(x)
                torch.cuda.synchronize()
                ms = triton.testing.do_bench(lambda: fn(x), warmup=10, rep=50)
                y = fn(x)
                torch.cuda.synchronize()
                diff = (y.float() - ref.float()).abs().max().item()
                print(f"{M},{N},{name},{float(ms):.6f},{_gbps(float(ms), x):.2f},{diff:.8g}")
            except Exception as exc:
                print(f"{M},{N},{name},ERROR,ERROR,{type(exc).__name__}: {exc}")
        del x, ref
