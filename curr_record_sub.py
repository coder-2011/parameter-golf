...
try:
    ...
    def flash_attn_3_func(q, k, v, causal=True):
        ...
        o = F.scaled_dot_product_attention(q2, k2, v2, is_causal=causal,
                                           enable_gqa=(k2.size(1) != q2.size(1)))
        return o.transpose(1, 2)
class Hyperparameters:
    # --- Data paths (auto-derived from vocab_size) ---
    ...
    # --- Parallel residuals (GPT-J style for layers >= this index) ---
    ...
    # --- Pre-quant AdamW TTT (runs on full-precision EMA model before GPTQ) ---
    prequant_ttt_enabled = bool(int(os.environ.get("PREQUANT_TTT_ENABLED", "1")))
    prequant_ttt_epochs = int(os.environ.get("PREQUANT_TTT_EPOCHS", 21))
    prequant_ttt_lr = float(os.environ.get("PREQUANT_TTT_LR", 5e-4))
    ...
    lbfgs_slot_iters = int(os.environ.get("LBFGS_SLOT_ITERS", 25))
    lbfgs_slot_history = int(os.environ.get("LBFGS_SLOT_HISTORY", 20))
    lbfgs_slot_focal = int(os.environ.get("LBFGS_SLOT_FOCAL", 128))
    lbfgs_slot_clamp = float(os.environ.get("LBFGS_SLOT_CLAMP", 5.0))
    lbfgs_slot_lr = float(os.environ.get("LBFGS_SLOT_LR", 1.0))

    # --- GPTQ quantization ---
    gptq_enabled = bool(int(os.environ.get("GPTQ_ENABLED", "1")))
    gptq_calibration_batches = int(os.environ.get("GPTQ_CALIBRATION_BATCHES", 256))
    ...
    # --- ETLB (embedding table logit bias) ---
...
# --- Muon optimizer (with MuonEq-R row normalization) ---
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "skip_weight,skip_weights,skip_gates,smear,dtg_gate,ve_layer_scales,"
        "ve_shared.scale,attn_gate,vr_lambda",
    ).split(",")
    if pattern
)
def _classify_param(name: str) -> str:
    """Classify a parameter name for quantization routing."""
...
# --- Validation data wrapper ---

class ValidationData:
    """Loads val tokens and builds sentencepiece LUTs on construction."""
...
def _get_shard_memmap(file: Path) -> np.ndarray:
    key = str(file)
...
class ShuffledSequenceLoader:
    """Memory-mapped shuffled sequence loader (SOTA approach)."""
    def __init__(self, h, device):
...
class CastedLinear(nn.Linear):
    """Linear layer that casts weights to input dtype on the fly."""
    def forward(self, x: Tensor) -> Tensor:
...
def restore_fp32_params(model: nn.Module) -> None:
    """Ensure CastedLinear weights and control tensors are in FP32."""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        _dt = torch.bfloat16 if q.dtype == torch.float32 else q.dtype
        y = flash_attn_3_func(q.to(_dt), k.to(_dt), v.to(_dt), causal=True).to(q.dtype)
        if self.use_xsa:
...
class Block(nn.Module):
    def __init__(self, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float,
        ...
        self.parallel = False  # set by GPT.__init__ for layers >= parallel_residual_start
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        if self.parallel:
            # GPT-J style: attn and MLP read from the same input
            ...
            # Standard sequential: MLP reads from post-attention
            x_out = x_in + self.attn_scale.to(dtype=x_in.dtype)[None, None, :] * attn_out
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # Optional embedding projection (if embedding_dim != model_dim)
        ...
        # XSA for last N layers
        if h.xsa_last_n > 0:
            for i in range(max(0, h.num_layers - h.xsa_last_n), h.num_layers):
                self.blocks[i].attn.use_xsa = True

        # Parallel residuals for layers >= parallel_residual_start
        if h.parallel_residual_start >= 0:
            for i in range(h.parallel_residual_start, h.num_layers):
                self.blocks[i].parallel = True

        # --- Depth recurrence: compute encoder/decoder layer indices ---
        ...
        # --- Skip connections with optional sigmoid gates ---
    ...
    def forward_logits(self, input_ids: Tensor) -> Tensor:
        """Forward pass returning logits (bsz, seq_len, vocab)."""
        ...
        # Pick encoder/decoder layer sequences (with or without looping)
...
def _loss_bpb(loss_sum, token_count, byte_count):
    """Convert accumulated loss/token/byte counts to (val_loss, val_bpb)."""
    val_loss = (loss_sum / token_count).item()
    val_bpb = val_loss / math.log(2.0) * (token_count.item() / byte_count.item())
    return val_loss, val_bpb

def eval_val(h, device, val_data, model):
    """Standard validation loss and BPB."""
    seq_len = h.eval_seq_len
    local_batch_tokens = h.val_batch_tokens // (h.world_size * h.grad_accum_steps)
    if local_batch_tokens < seq_len:
        raise ValueError(
            f"VAL_BATCH_SIZE must provide at least one sequence per rank; "
            f"got VAL_BATCH_TOKENS={h.val_batch_tokens}, WORLD_SIZE={h.world_size}, "
...
def eval_val_ttt(h, device, val_data, base_model, batch_seqs=32):
    """Test-time training: score-first TTT with sliding windows."""
    rank = h.rank
...
def timed_eval(label, fn, *args, **kwargs):
    """Run an eval function and log timing."""
...
def pre_quant_adamw_ttt(h, device, val_data, base_model):
    """Run AdamW TTT on the full-precision EMA model BEFORE GPTQ quantization.

    Key insight: SGD TTT on GPTQ-quantized models fails (+0.030 BPB) because
    quantized weights cannot be effectively fine-tuned. AdamW TTT on full-precision
    weights before quantization works because: (1) AdamW has per-parameter adaptive
    LR unlike SGD, (2) full-precision weights can be smoothly updated, (3) GPTQ then
    quantizes the already-adapted model.

    All ranks participate in parallel (Option C: per-epoch sync). Each rank processes
    an interleaved subset of chunks, then all ranks average parameters after each epoch.
    Expected speedup: ~8x on 8 GPUs (~80s vs ~635s).
    """
    ...
    log(f"prequant_ttt:start epochs={h.prequant_ttt_epochs} lr={h.prequant_ttt_lr} "
        f"freeze_blocks={h.prequant_ttt_freeze_blocks} wd={h.prequant_ttt_wd} "
        f"parallel={world_size}gpus")
    t0 = time.perf_counter()

    seq_len = h.eval_seq_len
    chunk_tokens = h.prequant_ttt_chunk_tokens
    total_tokens = val_data.val_tokens.numel() - 1
    num_chunks = (total_tokens + chunk_tokens - 1) // chunk_tokens

    # Freeze the first N blocks + embeddings
    frozen_params = set()
    for i in range(min(h.prequant_ttt_freeze_blocks, len(base_model.blocks))):
        for p in base_model.blocks[i].parameters():
            p.requires_grad_(False)
            frozen_params.add(id(p))
    base_model.tok_emb.weight.requires_grad_(False)
    frozen_params.add(id(base_model.tok_emb.weight))

    ttt_params = [p for p in base_model.parameters() if p.requires_grad and id(p) not in frozen_params]
    optimizer = torch.optim.AdamW(ttt_params, lr=h.prequant_ttt_lr,
                                   weight_decay=h.prequant_ttt_wd, fused=True)

    # Cosine annealing across epochs (PR #1633: eta_min = lr * 0.1)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=h.prequant_ttt_epochs, eta_min=h.prequant_ttt_lr * 0.1)

    # Compile the forward pass for faster TTT steps
    compiled_forward = torch.compile(base_model.forward, dynamic=False, fullgraph=True)
    log(f"prequant_ttt:compiled forward pass")
    ...
    for epoch in range(h.prequant_ttt_epochs):
        ...
        # Each rank processes an interleaved subset of chunks
        for ci in range(rank, num_chunks, world_size):
            ...
            if rank == 0 and ((ci + 1) % 40 == 0 or ci >= num_chunks - world_size):
                log(f"prequant_ttt:epoch {epoch+1}/{h.prequant_ttt_epochs} "
                    f"chunk {ci+1}/{num_chunks} lr={current_lr:.6f}")

        # Step the epoch-level LR scheduler
        scheduler.step()
        ...
        base_model.eval()
        with torch.no_grad():
            diag_loss, diag_bpb = eval_val(h, device, val_data, base_model)
        base_model.train()
    ...
    log(f"prequant_ttt:done in {elapsed:.1f}s ({world_size} gpus)")


# --- L-BFGS Causal SLOT (logit-space delta optimization) ---

def eval_val_sliding_lbfgs_slot(h, device, val_data, base_model, batch_seqs=1):
    """Sliding window evaluation with L-BFGS logit-space SLOT optimization.

    Score-first protocol: score each window with the current delta, then optimize
    the delta for the next window. Delta is a shared [vocab_size] vector that is
    warm-started across windows and clamped to +/- clamp_val.
    """
    base_model.eval()
    logits_fn = torch.compile(base_model.forward_logits, dynamic=False, fullgraph=True)
    seq_len = h.eval_seq_len
    ...
    # Shared delta vector in logit space, warm-started across windows
    delta = torch.zeros(h.vocab_size, device=device, dtype=torch.float32, requires_grad=True)
    clamp_val = h.lbfgs_slot_clamp
    focal_tokens = h.lbfgs_slot_focal

    log(f"lbfgs_slot:start windows={len(my_windows)} iters={h.lbfgs_slot_iters} "
        f"history={h.lbfgs_slot_history} focal={focal_tokens} clamp={clamp_val}")

    # Process windows one at a time, computing logits on-the-fly to avoid OOM
    for window_idx, ws in enumerate(my_windows):
        we = min(ws + seq_len, total_tokens)
        wlen = we - ws
        chunk = val_data.val_tokens[ws:we + 1].to(dtype=torch.int64, device=device)
        ...
        # Pad to seq_len for compiled model
        if wlen < seq_len:
        ...
        with torch.inference_mode():
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                logits = logits_fn(x_padded)
            logits_i = logits[0, :wlen].float()  # [wlen, vocab]

        s = 0 if ws == 0 else context_size

        # --- Score phase: apply current delta and score ---
        with torch.no_grad():
            scored_logits = logits_i[s:wlen] + delta.detach()  # [scored_len, vocab]
            nll = F.cross_entropy(
                scored_logits.float(),
                y[s:wlen],
                reduction="none",
        ...
        # --- Optimize phase: optimize delta for next window using this window's data ---
        if h.lbfgs_slot_iters > 0 and wlen > s:
            # Use focal tokens (last N tokens of the scored region) for optimization
            opt_start = max(s, wlen - focal_tokens)
            opt_logits = logits_i[opt_start:wlen].detach()  # [opt_len, vocab]
            opt_targets = y[opt_start:wlen]  # [opt_len]

            # Reset delta grad but keep values (warm start)
            delta_opt = delta.detach().clone().requires_grad_(True)
            lbfgs = torch.optim.LBFGS(
                [delta_opt],
                lr=h.lbfgs_slot_lr,
                max_iter=h.lbfgs_slot_iters,
                history_size=h.lbfgs_slot_history,
                line_search_fn="strong_wolfe",
            )

            def closure():
                lbfgs.zero_grad()
                adjusted = opt_logits + delta_opt
                loss = F.cross_entropy(adjusted.float(), opt_targets, reduction="mean")
                loss.backward()
                return loss

            lbfgs.step(closure)

            # Update delta with clamping, warm-start for next window
            with torch.no_grad():
                delta.copy_(delta_opt.clamp(-clamp_val, clamp_val))
...
class Optimizers:
    """Groups all optimizers and handles LR scheduling."""
...
# --- GPTQ quantization (SDClip + Hessian-guided) ---

def collect_hessians(model, train_loader, h, device, n_calibration_batches=64):
    """Collect H = X^T X hessians for all CastedLinear layers using forward hooks."""
    hessians = {}
    hooks = []
    def make_hook(name):
    ...
    # Also collect Hessian for embedding table (if tied)
...
def gptq_quantize_weight(w, H, clip_sigmas=3.0, clip_range=63, block_size=128):
    """GPTQ with SDClip: scale = clip_sigmas * std(row) instead of percentile search."""
    ...
    # Permute columns by Hessian diagonal (largest first)
    ...
    # SDClip: scale = clip_sigmas * row_std / clip_range
...
def gptq_mixed_quantize(state_dict, hessians, h):
    """Apply GPTQ with SDClip to all large weight tensors (including embeddings)."""
    result = {}
    meta = {}
...
def dequantize_mixed(result, meta, template_sd):
    """Dequantize from GPTQ result back to float state_dict."""
    out = {}
...
def _compress(data, compressor):
    """Compress data with byte shuffling + chosen compressor."""
...
def _decompress(data, compressor):
    """Decompress data with chosen compressor + byte unshuffling."""
    if compressor == "lzma":
...
def serialize(h, base_model, code):
    """Quantize + compress model and save to disk."""
    ...
    if h.is_main_process:
        ...
        # NOTE: If total exceeds 16MB (16,000,000 bytes), the code must be LZMA-wrapped
        # for submission. The model itself fits (~15.97MB); it's the code size (~81KB)
        # that pushes it over. Use: lzma.compress(code.encode()) in the submission script.
        if bytes_total > 16_000_000:
            log(f"WARNING: submission {bytes_total} bytes exceeds 16MB limit by "
                f"{bytes_total - 16_000_000} bytes. Code needs LZMA wrapping for submission.")
...
def train_model(h, device, val_data):
    """Train the model and return (base_model, compiled_model)."""
    ...
    # Warmup phase (warmup then reset)
    if h.warmup_steps > 0:
        ...
        # Optional loop warmup (activates depth recurrence during warmup too)
        if h.num_loops > 0:
    ...
    while True:
        ...
        # Activate depth recurrence at enable_looping_at fraction
...
def train_and_eval(h, device):
    """Full pipeline: train, quantize, evaluate."""
    random.seed(h.seed)
    ...
    # Pre-quant AdamW TTT: adapt full-precision model on val data before GPTQ
    if h.prequant_ttt_enabled:
        del compiled_model
        torch._dynamo.reset()
        torch.cuda.empty_cache()
        pre_quant_adamw_ttt(h, device, val_data, base_model)
        # Re-compile after TTT for post-TTT eval
        compiled_model = torch.compile(base_model, dynamic=False, fullgraph=True)
    ...
    # L-BFGS Causal SLOT: logit-space delta optimization during sliding window eval
    if h.lbfgs_slot_enabled and h.sliding_window_enabled:
        torch._dynamo.reset()
        torch.cuda.empty_cache()
        slot_model = deserialize(h, device)
        if h.num_loops > 0:
            slot_model.looping_active = True
        timed_eval("quantized_lbfgs_slot", eval_val_sliding_lbfgs_slot,
                   h, device, val_data, slot_model)
        del slot_model
...
def main() -> None:
    ...
    # NCCL timeout: all ranks active during TTT now, no long single-rank waits
    os.environ.setdefault("TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC", "600")
    os.environ.setdefault("NCCL_TIMEOUT", "600000")
    if distributed:
